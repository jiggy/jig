...
| Native Bun dependency preparation | `b2ebfe4` through `f99d799`, especially the `bun-native-*` files and reviews 175, 186–188, and 190–195 at `6aa93e1` | A contained, networkless, script-disabled preparation path worked, but its store and recovery lifecycle were too large for the first direct alpha. | A real package campaign proves that author-prepared dependency closure is inadequate and renewed host preparation is explicitly approved. |
