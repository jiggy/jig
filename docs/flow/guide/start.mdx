...
explain. The same boundary can later contain code, Agent judgment, or both;
starting with code makes that boundary easy to see.
...
The package contains one `FLOW.<ext>` implementation, such as `FLOW.ts`,
`FLOW.py`, or `FLOW.md`. Add optional metadata, resources and contracts when
the method needs them. See
[Package/0](../spec/package-format.md) for the exact metadata and layout.

A Markdown implementation needs a compatible interpreter. Code implementations
likewise need support from the chosen host.

## Pick your authoring path

| Your environment | Follow this path |
| --- | --- |
| Markdown | [Markdown authoring](./markdown.md), including input, child calls, outcomes, and channels. Check [Skill compatibility](./skills.md) when bringing an existing Skill. |
| TypeScript or JavaScript | [SDK tutorial](./typescript.md), including a local protocol exercise and expected output. |
| Python | [Python SDK tutorial](./python.md), including a local protocol exercise and expected output. |
| A hosted project | Follow your host's setup. For example, [Jig's first Flow](https://jig.md/guide/) initializes a package with its qualified SDK revision. |
...
expects a Run/0 host exchange on stdin and stdout. Use the complete setup in
...
- [See one caller with three implementations](https://jig.md/guide/request-triage) in a Jig example.
...
- [Use the Run SDK](../spec/run-sdk.md) for calls through host-bound slots.
