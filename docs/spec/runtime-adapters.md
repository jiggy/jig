...
An exact-code recipe may request the one v1 `root-process-mappings` runtime
predicate. A Sandbox Backend may realize it only as one read-only,
non-traversable pseudo-file at a recipe-pinned sandbox path whose contents are
generated by the kernel for the activation root process. The subject remains
that same root process across final `exec`. The view exposes neither mapped
memory contents nor data or control for a host or sibling process. A descendant
may observe only the same pinned root-owned view; it does not receive a
retargeted per-descendant `self` view.

This predicate is host runtime support, not part of the immutable Runtime
Support Closure, Package/1 content, an attachment, capability, setting,
Binding grant, or author-selectable requirement. It implies no
pseudo-filesystem root, directory contents, mutable control, descriptor,
environment, or additional kernel file. The closed plan and realized receipt
name the predicate, sandbox path, root-subject mechanism, access mode, and
successful Backend probe. If any invariant cannot be proven, the recipe is
unavailable. No other kernel pseudo-file is admitted in v1.
...
18. Descendant creation never widens authority or creates an implicit owner;
    ...
    cleanup.
19. `root-process-mappings`, when required, follows the activation root across
    final `exec`; a descendant sees only that same root view, broad or writable
    pseudo-filesystems and host-process data remain unreachable, and the view
    is fenced with the owner.
