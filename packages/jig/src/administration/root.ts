...
export function normalizeStartRootRunRequest(value: unknown): StartRootRunRequest {
  const root = exactRecord(
    snapshotJson(value, 'start Run request'),
    ['submissionId', 'target', 'input'],
    'start Run request',
  )
  const submissionLength =
    typeof root.submissionId === 'string' ? scalarLength(root.submissionId) : 0
...
export function normalizeRootRunStatusRequest(value: unknown): RootRunStatusRequest {
  const root = exactRecord(
    snapshotJson(value, 'Run status request'),
    ['runId'],
    'Run status request',
  )
...
function copyJson(
  ...
  try {
    ...
    if (keys.length > JSON_1_LIMITS.containerEntries)
      throw new TypeError(`${label} has too many members`)
