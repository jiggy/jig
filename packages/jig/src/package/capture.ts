...
export function createCapturedPackage(
  ...
  return Object.freeze({
    ...
    async read(path: string, maximumBytes = MAX_FILE_BYTES): Promise<Uint8Array> {
      ...
      if (offset !== file.size)
        invalid('PACKAGE_STAGE_CHANGED', `captured file ${path} became short`, path)
      return bytes
    },
...
function requireCapturedFile(files: ReadonlyMap<string, CapturedFile>, path: string): CapturedFile {
...
async function openDirectoryRoot(source: string): Promise<OpenRoot> {
  ...
  try {
    ...
    if (isResourceError(error))
      resourceExhausted(error, 'cannot inspect package directory', requestedPath)
    unavailable(
      'PACKAGE_SOURCE_IO',
      `cannot inspect package directory: ${errorText(error)}`,
      requestedPath,
    )
  }
  if (observed.isSymbolicLink())
    invalid('PACKAGE_ROOT', 'package source root must not be a symlink', requestedPath)
  if (!observed.isDirectory())
    invalid('PACKAGE_ROOT', 'package source is not a directory', requestedPath)
  ...
  try {
    ...
    if (isResourceError(error))
      resourceExhausted(error, 'cannot open package directory', requestedPath)
    if (isEntryRace(error))
      sourceChanged('package source root changed while it was opened', requestedPath)
    unavailable(
      'PACKAGE_SOURCE_IO',
      `cannot open package directory: ${errorText(error)}`,
      requestedPath,
    )
  }
  try {
...
async function duplicateDirectoryRoot(
  ...
  try {
    ...
    if (!information.isDirectory())
      invalid('PACKAGE_ROOT', 'opened package source is not a directory', sourceRoot)
...
async function verifyRootPath(root: OpenRoot): Promise<void> {
  ...
  try {
    ...
    if (isResourceError(error))
      resourceExhausted(error, 'cannot verify package directory', root.requestedPath)
...
async function rejectKnownOversizeFile(
  ...
  try {
    ...
    if (isResourceError(error))
      resourceExhausted(error, 'cannot probe package source file', logicalPath)
    unavailable(
  ...
  while (true) {
    ...
    while (written < bytesRead) {
      ...
      if (result.bytesWritten === 0)
        unavailable('PACKAGE_CAPTURE_IO', 'package snapshot write made no progress')
      ...
      try {
        if (entry.information.isDirectory()) {
          ...
          if (directories.has(key))
            sourceChanged('directory alias appeared during verification', logicalPath)
          directories.add(key)
          await walk(entry.handle, logicalPath)
        } else {
...
function directoryEntryName(entry: unknown): Uint8Array {
  ...
  unavailable(
    'PACKAGE_CAPTURE_UNAVAILABLE',
    'runtime cannot enumerate raw directory-entry names incrementally',
  )
}
...
async function openDirectoryEntry(
  ...
  if (observed.isSymbolicLink())
    invalid('PACKAGE_SYMLINK', 'symlinks are not package files', logicalPath)
...
function noteHardlink(
  ...
  if (prior === undefined) {
    ...
    prior.observed += 1
    if (prior.expected !== information.nlink)
      sourceChanged('hardlink count changed during capture', logicalPath)
  }
}
