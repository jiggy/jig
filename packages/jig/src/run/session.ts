...
export class RunHostSession {
  ...
  private startTermination(): void {
    ...
    this.own(
      this.process.terminate().catch((error) => {
  ...
  private finish(exit: ExactComponentExit | undefined): RunHostTerminal {
    ...
    if (this.rootResponse.kind === 'failure') {
      return failure(this.rootResponse.code, this.rootResponse.message, this.rootResponse.details)
    }
...
function validateLimits(limits: RunHostLimits): void {
  for (const [name, value] of Object.entries(limits)) {
    if (!Number.isSafeInteger(value) || value < 0)
      throw new TypeError(`invalid Run host limit ${name}`)
  }
...
function parseEnvelope(value: JsonValue): ParsedEnvelope {
  ...
  if (hasMethod) {
    ...
    if (hasParams && (object.params === null || typeof object.params !== 'object')) {
...
function parseErrorPayload(value: JsonValue): ParsedError['error'] {
  ...
  requireExactKeys(
    object,
    Object.hasOwn(object, 'data') ? ['code', 'message', 'data'] : ['code', 'message'],
  )
  if (!Number.isSafeInteger(object.code)) throw new Error('invalid error code')
  if (
    typeof object.message !== 'string' ||
    scalarLength(object.message) < 1 ||
...
function normalizeOperationFailure(value: RunHostOperationFailure): RunHostOperationFailure {
  if (
    !WIRE_FAILURE_CODES.has(value.code) ||
    typeof value.message !== 'string' ||
    scalarLength(value.message) < 1 ||
    scalarLength(value.message) > 1_024
  ) {
