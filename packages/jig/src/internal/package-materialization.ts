...
export async function recoverPrivatePackageMaterializationAllocation(
  ...
  try {
    ...
    if (entries.length === 1 && entries[0] === PACKAGE_NAME) {
    ...
    } else if (entries.length !== 0 && !(entries.length === 1 && entries[0] === DISPOSING_NAME)) {
...
async function openProtectedParent(
  ...
  try {
    ...
    if (
      ...
      information.uid !== BigInt(process.geteuid()) ||
...
async function openPackageDirectory(
  ...
  try {
    requireOwnedDirectory(
      await current.stat({ bigint: true }),
      filesystem,
      [expectedMode],
      'package root',
    )
...
function requireOwnedDirectory(
  ...
  if (
    ...
    information.uid !== filesystem.uid ||
    information.dev !== filesystem.dev ||
    !modes.includes(fileMode(information))
  )
...
function parseDigest(value: unknown): string {
  if (typeof value !== 'string' || !DIGEST.test(value)) {
    throw new TypeError(
      'materialization package digest must be sha256 plus 64 lowercase hex digits',
    )
  }
  return value
}
