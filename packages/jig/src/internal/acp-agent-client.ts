...
export async function runPrivateAcpTurn(
  ...
  const app = acp.client({ name: "jig" }).onRequest(
  ...
  return await app.connectWith(stream, async (agent) => {
    const initialized = await agent.request(acp.methods.agent.initialize, {
      ...
      clientCapabilities: request.authentication?.clientAuthCapabilities === undefined
        ? {}
    ...
    if (request.authentication !== undefined) {
      ...
      if (!initialized.authMethods?.some(({ id }) => id === methodId)) {
        throw new PrivateAcpProtocolError("the ACP agent did not advertise the required authentication method");
      }
      await agent.request(
    ...
    try {
      ...
      session.dispose();
      if (canClose) {
        await boundedClose(agent.request(
