...
function requireProtectedDirectory(
  ...
  if (!information.isDirectory())
    invalid('PACKAGE_ARTIFACT_STORE', `${description} must be a directory`)
  if (information.uid !== expectedUid) {
    invalid(
      'PACKAGE_ARTIFACT_STORE_OWNER',
      `${description} must be owned by the Jig coordinator identity`,
    )
  }
  if ((information.mode & 0o022n) !== 0n) {
    invalid(
      'PACKAGE_ARTIFACT_STORE_PERMISSIONS',
      `${description} must not be writable by group or others`,
    )
  }
}
...
async function capturePackageArtifactFile(
  ...
  try {
    ...
    if (!sameStableFile(before, after))
      artifactCorrupt(`${sourceLabel} changed while it was verified`)
...
async function writeCapturedArchive(
  ...
  const hash = createHash('sha256')
  const iterator = encodePackage1(captured.files, (file) => captured.stream(file.path))[
...
async function digestCapturedPackage(captured: CapturedPackage): Promise<string> {
  try {
    return await packageDigest(captured.files, (file) => captured.stream(file.path))
  } catch (error) {
    if (error instanceof CheckError || isResourceError(error)) throw error
    invalid(
      'PACKAGE_ARTIFACT_SOURCE_INVALID',
...
function bigintToNumber(value: bigint, description: string): number {
  if (value > BigInt(Number.MAX_SAFE_INTEGER))
    artifactCorrupt(`stored artifact ${description} is too large`)
  return Number(value)
}
