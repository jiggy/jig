...
import { access, readFile, readdir, realpath, stat, statfs } from 'node:fs/promises'
...
async function observe(
  ...
  if (
    delegatedCgroup === CGROUP_ROOT ||
    !delegatedCgroup.startsWith(`${CGROUP_ROOT}/`) ||
  ...
  if (
    retained !== undefined &&
    (retained.currentCgroup !== currentCgroup ||
      retained.delegatedCgroup !== delegatedCgroup ||
      retained.payloadUid !== uid ||
      retained.payloadGid !== gid)
  ) {
    throw new Error('the inherited rootless Linux authority changed')
  }
  if (
    (await dependencies.resolve(currentCgroup)) !== currentCgroup ||
  ...
  if (
    ...
    !delegatedInformation.isDirectory() ||
    currentInformation.uid !== uid ||
    delegatedInformation.uid !== uid
  ) {
  ...
  for (const path of activeRunCgroups) {
    if (
      typeof path !== 'string' ||
      posix.dirname(path) !== delegatedCgroup ||
  ...
  if (
    !childDirectories.includes(currentName) ||
    childDirectories.some((name) => !allowedChildren.has(name)) ||
    (retained === undefined && childDirectories.length !== 1)
  ) {
    throw new Error(
      'the immediate parent cgroup is not exclusive to this payload and its active Runs',
    )
  }
  ...
  await dependencies.requireAccess(
    `${delegatedCgroup}/cgroup.procs`,
    constants.R_OK | constants.W_OK,
  )
  await dependencies.requireAccess(
    `${delegatedCgroup}/cgroup.subtree_control`,
    constants.R_OK | constants.W_OK,
  )
  ...
  if (
    !bubblewrapInformation.isFile() ||
    (bubblewrapInformation.mode & 0o111) === 0 ||
