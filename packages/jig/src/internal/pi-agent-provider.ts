...
export interface PrivatePiApiKeyAgentConfiguration extends PrivatePiAgentSupport {
...
export interface PrivatePiSubscriptionAgentConfiguration extends PrivatePiAgentSupport {
...
function requireSubscriptionCredential(value: Uint8Array, provider: string): Uint8Array {
  ...
  if (
    !isRecord(entry) ||
    !exactKeys(entry, ['access', 'expires', 'refresh', 'type']) ||
    entry.type !== 'oauth' ||
    typeof entry.access !== 'string' ||
    entry.access.length === 0 ||
    entry.access.includes('\0') ||
    entry.refresh !== '' ||
...
function projectSubscriptionCredential(value: Uint8Array, provider: string): Uint8Array {
  ...
  const entry = parsed[provider]
  const allowedKeys =
    provider === 'openai-codex' && isRecord(entry) && Object.hasOwn(entry, 'accountId')
...
function ordinaryJson(value: unknown, depth = 0): boolean {
  ...
  return (
    ...
    Object.entries(value).every(
      ([key, item]) => key.length > 0 && !key.includes('\0') && ordinaryJson(item, depth + 1),
    )
  )
}
...
async function readCredentialFile(path: string): Promise<Uint8Array> {
  ...
  if (
    ...
    information.nlink !== 1 ||
    information.size <= 0 ||
    information.size > MAX_CREDENTIAL_BYTES ||
    uid === undefined ||
    information.uid !== uid ||
