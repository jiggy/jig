...
export async function createPrivateAcpAgentProvider(
  ...
  const sessionMeta =
    value.sessionMeta === undefined
      ? undefined
      : requireJsonObject(snapshot(value.sessionMeta, 'ACP session metadata'))
  const authentication =
    value.authentication === undefined ? undefined : normalizeAuthentication(value.authentication)
  const startupInput =
    value.startupInput === undefined ? undefined : normalizeStartupInput(value.startupInput)
  ...
  const readOnlyMounts = Object.freeze(
...
function normalizeAuthentication(
  ...
  const clientAuthCapabilities =
    value.clientAuthCapabilities === undefined
      ? undefined
  ...
  return Object.freeze({
    ...
    ...(clientAuthCapabilities === undefined
      ? {}
      : {
          clientAuthCapabilities: clientAuthCapabilities as NonNullable<
            acp.ClientCapabilities['auth']
          >,
        }),
    identity,
  })
