...
async function superviseConnected(control: Socket, startupDeadlineUnixMs: number): Promise<void> {
  ...
  try {
    ...
    if (await exists(`${runCgroup}/memory.swap.max`)) await writeFile(`${runCgroup}/memory.swap.max`, "0\n");
    ...
    safeSend(control, { type: "prepared", runCgroup, supervisorPid: process.pid } satisfies PreparedMessage);
    await admission;
...
async function innerMain(command: readonly string[]): Promise<void> {
  if (command.length === 0 || !absolute(command[0]!)) throw new Error("invalid rootless inner command");
  ...
  const child = spawn(command[0]!, command.slice(1), { cwd: "/work", env: process.env, stdio: "inherit" });
  const exit = await childClose(child);
  if (exit.signal !== null) process.kill(process.pid, exit.signal as NodeJS.Signals);
  process.exitCode = exit.code ?? 1;
}
...
function bubblewrapArguments(configuration: Configuration): string[] {
  ...
  for (const [name, value] of Object.entries(configuration.environment).sort(([a], [b]) => a.localeCompare(b))) {
...
async function claimActive(configuration: Configuration): Promise<"created" | "cancelled"> {
  ...
  if (claim.allocationDigest !== configuration.ownerStateAllocationDigest ||
      claim.kind !== "private-linux-owner-claim/1" || claim.token !== configuration.ownerToken ||
...
async function requireActiveClaim(directory: string, token: string, allocationDigest: string): Promise<void> {
  const claim = JSON.parse(await readFile(`${directory}/claim.json`, "utf8")) as Record<string, unknown>;
  if (claim.kind !== "private-linux-owner-claim/1" || claim.state !== "active" || claim.token !== token ||
...
async function requireOwnerRecord(configuration: Configuration): Promise<void> {
  const value = JSON.parse(await readFile(`${configuration.ownerStateDirectory}/owner.json`, "utf8")) as Record<string, unknown>;
  if (value.kind !== "private-linux-owner-state/1" ||
...
async function persistFinal(configuration: Configuration, terminal: TerminalMessage): Promise<void> {
...
function requireStart(value: unknown): Configuration {
  if (recordType(value) !== "start" || value === null || typeof value !== "object" ||
  ...
  if (configuration === null || typeof configuration !== "object" || Array.isArray(configuration) ||
      ...
      !absolute(configuration.bunPath) || !absolute(configuration.bubblewrapPath) ||
      !absolute(configuration.supervisorPath) ||
      typeof configuration.mechanismDigest !== "string" || !DIGEST.test(configuration.mechanismDigest) ||
      typeof configuration.ownerDigest !== "string" || !DIGEST.test(configuration.ownerDigest) ||
      typeof configuration.sealedPlanDigest !== "string" || !DIGEST.test(configuration.sealedPlanDigest) ||
      ...
      configuration.command.length === 0 || !absolute(configuration.command[0]!) ||
...
function validLimits(value: unknown): value is Limits {
  ...
  const keys = [
    "cancellationGraceMs", "cleanupTimeoutMs", "cpuPeriodMicros", "cpuQuotaMicros",
    "deadlineUnixMs", "memoryBytes", "pids",
  ];
  return Object.keys(limits).sort().join("\0") === keys.sort().join("\0") &&
    positiveInteger(limits.memoryBytes) && positiveInteger(limits.pids) &&
    positiveInteger(limits.cpuQuotaMicros) && positiveInteger(limits.cpuPeriodMicros) &&
    nonnegativeInteger(limits.deadlineUnixMs) && positiveInteger(limits.cancellationGraceMs) &&
...
function validMounts(value: unknown): value is readonly Mount[] {
  return Array.isArray(value) && value.every((mount) => {
    ...
    return Object.keys(record).sort().join("\0") === "destination\0source" &&
      typeof record.source === "string" && absolute(record.source) &&
      typeof record.destination === "string" && absolute(record.destination);
  });
}
...
async function readCounters(path: string): Promise<Record<string, number>> {
  ...
  for (const line of (await readFile(path, "utf8")).trim().split("\n")) {
    ...
    if (name === undefined || !Number.isSafeInteger(value) || value < 0) throw new Error(`invalid counter in ${path}`);
    result[name] = value;
  }
  return result;
}
...
function absolute(path: string): boolean {
  return typeof path === "string" && path.startsWith("/") && !path.includes("\0") && posix.normalize(path) === path;
}
