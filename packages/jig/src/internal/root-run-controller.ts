...
async function startOrResumeCurrentExecution(
  ...
  try {
    ...
    if (work.lifecycle.plan === undefined) {
      ...
      stop.narrow(
        Math.min(work.intent.deadlineUnixMs, activationStartedUnixMs + recipe.wallClockCeilingMs),
      )
      if (stop.terminal !== undefined) {
        return terminal(await settleWithoutPlan(input, work, stop.terminal))
      }
    ...
    if (work.lifecycle.sandbox !== undefined || work.lifecycle.provisional !== undefined) {
      return await recoverCurrentExecution(input, work, observedTerminal ?? stop.terminal, error)
    }
    if (work.lifecycle.plan !== undefined) {
...
async function recoverCurrentExecution(
  ...
  let work = initial
  let confirmedFence =
    work.lifecycle.fence === undefined ? undefined : parseFence(work.lifecycle.fence.value).receipt
  ...
  if (work.lifecycle.provisional === undefined) {
    work = await advanceCheckpoint(
      ...
      'provisional',
      (knownTerminal ??
        terminalAfterRecoveredFence(confirmedFence, fallbackError)) as unknown as JsonValue,
    )
  }
  try {
    return terminal(await releaseAdmitAndClose(input, work))
...
async function settleBeforeSandbox(
  ...
  work = await advanceCheckpoint(input, work, 'provisional', provisional as unknown as JsonValue)
  try {
    return terminal(await releaseAdmitAndClose(input, work))
  } catch (error) {
    if (error instanceof PrivateLinuxFenceUnconfirmedError) {
...
async function reproduceRecipe(
  ...
  if (target === undefined) throw new Error('durable root Run target is absent from its candidate')
  const request = target.request
  if (request.digest !== work.intent.requestDigest || target.disposition.state !== 'ready') {
...
function parsePlan(value: JsonValue): PrivateDirectRootPlanRecord {
  ...
  if (
    ...
    !Number.isSafeInteger(record.activationStartedUnixMs) ||
...
async function requirePlanMatches(
  ...
  const hexadecimal = runHex(work.run.runId)
  if (
    plan.requestDigest !== work.intent.requestDigest ||
    plan.recipeDigest !== recipe.digest ||
    plan.recipeDigest !== work.intent.recipeDigest ||
    ...
    plan.effectiveDeadlineUnixMs !==
      Math.min(work.intent.deadlineUnixMs, plan.activationStartedUnixMs + recipe.wallClockCeilingMs)
  ) {
...
class FirstStop {
  ...
  private readonly abort = (): void => {
    this.terminal ??= failedPrivateRootTerminal(
      'CANCELLED',
      'root Run was cancelled before dispatch',
    )
  ...
  narrow(deadlineUnixMs: number): void {
    if (
      ...
      deadlineUnixMs < 0 ||
      deadlineUnixMs > this.deadlineUnixMs
    ) {
  ...
  private arm(): void {
    ...
    if (remaining <= 0) {
      this.terminal ??= failedPrivateRootTerminal(
        'DEADLINE_EXCEEDED',
        'root Run deadline elapsed before dispatch',
      )
      if (this.startupEnforcement && !this.enforcement.signal.aborted) {
...
function operationInput(input: RootExecutionInput, parent: PrivateReacquiredRootExecutionWork) {
  return {
    projectRoot: input.projectRoot,
    packageStoreRoot: input.packageStoreRoot,
    parent,
