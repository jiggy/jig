    ...
    this.#options = Object.freeze({
      ...
      supervisorPath: requireAbsolute(
        options.supervisorPath ??
          join(dirname(sourcePath), `linux-rootless-supervisor${extension}`),
        'supervisor path',
      ),
  ...
  async seal(
    ...
    const result: PrivateLinuxSealedOwner = Object.freeze({
      ...
      admit: (
        ...
        if (admitted)
          return Promise.reject(new TypeError('sealed rootless Linux owner was already admitted'))
        admitted = true
  ...
  async launch(
    ...
    try {
      ...
      control = await Promise.race([
        ...
        closed.then((exit) => {
          throw new Error(
            `rootless supervisor exited before connecting (${exit.code ?? exit.signal})`,
          )
        }),
      ])
      server.close()
      ...
      if (closed !== undefined) {
        await withTimeout(
          closed,
          data.identity.cleanupTimeoutMs + this.#options.startupTimeoutMs,
          'rootless supervisor cleanup',
        ).catch(() => undefined)
      }
...
export function normalizePrivateLinuxOwnerStateAllocationIdentity(
  ...
  if (
    record.kind !== 'private-linux-owner-state-allocation/1' ||
    typeof record.digest !== 'string' ||
    !DIGEST.test(record.digest) ||
    typeof record.parent !== 'string' ||
    !canonicalAbsolute(record.parent) ||
    typeof record.parentDevice !== 'string' ||
    !nonnegativeIntegerText(record.parentDevice) ||
    typeof record.parentInode !== 'string' ||
    !positiveIntegerText(record.parentInode) ||
    typeof record.name !== 'string' ||
    !OWNER_NAME.test(record.name) ||
    typeof record.directory !== 'string' ||
    record.directory !== join(record.parent, record.name) ||
    typeof record.ownerToken !== 'string' ||
    !TOKEN.test(record.ownerToken)
  ) {
  ...
  if (
    record.digest !==
    privateDomainDigest('JIG-Rootless-Linux-Owner-Allocation/1', fields as unknown as JsonValue)
  )
...
async function allocationCancellationRecord(
  ...
  try {
    ...
    if (
      ...
      record.allocationDigest !== allocation.digest ||
      record.token !== allocation.ownerToken ||
      ...
      !canonicalCgroup(record.runCgroup)
    ) {
      throw error
    }
    return {
...
export function normalizePrivateLinuxSealedOwnerIdentity(
  ...
  const record = exactRecord(
    ...
    [
      ...
      'deadlineUnixMs',
      'delegatedCgroup',
      'delegatedCgroupDevice',
      'delegatedCgroupInode',
      'digest',
      'kind',
      'mechanismDigest',
      'nonce',
      ...
      'ownerStateName',
      'ownerStateParent',
      'ownerStateParentDevice',
      'ownerStateParentInode',
      'ownerToken',
      'runCgroup',
      'runId',
      'sealedPlanDigest',
  ...
  if (
    record.kind !== 'private-linux-sealed-owner/1' ||
    typeof record.digest !== 'string' ||
    !DIGEST.test(record.digest) ||
    ...
    typeof record.delegatedCgroup !== 'string' ||
    !canonicalCgroup(record.delegatedCgroup) ||
    typeof record.delegatedCgroupDevice !== 'string' ||
    !nonnegativeIntegerText(record.delegatedCgroupDevice) ||
    typeof record.delegatedCgroupInode !== 'string' ||
    !positiveIntegerText(record.delegatedCgroupInode) ||
    typeof record.runCgroup !== 'string' ||
    ...
    typeof record.ownerStateParent !== 'string' ||
    !canonicalAbsolute(record.ownerStateParent) ||
    typeof record.ownerStateParentDevice !== 'string' ||
    !nonnegativeIntegerText(record.ownerStateParentDevice) ||
    typeof record.ownerStateParentInode !== 'string' ||
    !positiveIntegerText(record.ownerStateParentInode) ||
    typeof record.ownerStateName !== 'string' ||
    !OWNER_NAME.test(record.ownerStateName) ||
    typeof record.ownerStateDirectory !== 'string' ||
    record.ownerStateDirectory !== join(record.ownerStateParent, record.ownerStateName) ||
    typeof record.ownerStateDevice !== 'string' ||
    !nonnegativeIntegerText(record.ownerStateDevice) ||
    typeof record.ownerStateInode !== 'string' ||
    !positiveIntegerText(record.ownerStateInode) ||
    typeof record.ownerStateAllocationDigest !== 'string' ||
    !DIGEST.test(record.ownerStateAllocationDigest)
  ) {
  ...
  if (
    record.digest !==
    privateDomainDigest('JIG-Rootless-Linux-Sealed-Owner/1', fields as unknown as JsonValue)
  )
...
export function normalizePrivateLinuxPreparedOwnerIdentity(
  ...
  if (
    record.kind !== 'private-linux-prepared-owner/1' ||
    typeof record.digest !== 'string' ||
    record.digest !==
...
export function normalizePrivateLinuxConfirmedEnforcementReceipt(
  ...
  if (
    record.kind !== 'private-linux-confirmed-enforcement/1' ||
    typeof record.ownerDigest !== 'string' ||
    !DIGEST.test(record.ownerDigest) ||
...
export function normalizePrivateLinuxOwnerStateReleaseReceipt(
  ...
  if (
    record.kind !== 'private-linux-owner-state-release/1' ||
    typeof record.digest !== 'string' ||
    !DIGEST.test(record.digest) ||
    typeof record.allocationDigest !== 'string' ||
    !DIGEST.test(record.allocationDigest) ||
...
export async function releasePrivateLinuxOwnerState(
  ...
  if (
    String(directoryInformation.dev) !== reference.directoryDevice ||
...
function releaseReferenceForAllocation(ownerValue: unknown, proofValue: unknown): ReleaseReference {
  ...
  if (proof.allocationDigest !== allocation.digest)
    throw new TypeError('owner cancellation does not match allocation')
  return {
    allocationDigest: allocation.digest,
...
function releaseReferenceForOwner(ownerValue: unknown, proofValue: unknown): ReleaseReference {
  ...
  if (proof.ownerDigest !== prepared.digest)
    throw new TypeError('enforcement receipt does not match owner')
  return {
...
async function observeInitialMechanism(
  options: NormalizedOptions,
): Promise<InitialMechanismObservation> {
...
async function observeMechanismWithAuthority(
  ...
  if (
    ...
    bunHostLibraryPath !== options.bunHostLibraryPath ||
  ...
  const support = Object.freeze({
    ...supportFields,
    digest: privateDomainDigest(
      'JIG-Rootless-Linux-Mechanism/1',
...
async function observeLinuxBootId(): Promise<string> {
  ...
  if (
    resolved !== BOOT_ID_PATH ||
    !information.isFile() ||
    information.isSymbolicLink() ||
    (Number(information.mode) & 0o222) !== 0 ||
    BigInt(filesystem.type) !== PROC_SUPER_MAGIC ||
    !text.endsWith('\n') ||
    text.indexOf('\n') !== text.length - 1
  ) {
...
export function requirePrivateLinuxMechanismUnchanged(
  ...
  if (
    current.support.digest !== sealed.support.digest ||
...
function supervisorConfiguration(data: SealedOwnerData): object {
  return Object.freeze({
    ...
    readOnlyMounts: data.sealedPlan.readOnlyMounts.map(({ source, destination }) => ({
      source,
      destination,
    })),
...
function snapshotPlan(value: PrivateLinuxLaunchPlan): SealedLaunchPlan {
  ...
  if (
    normalizedLimits.deadlineUnixMs + normalizedLimits.cancellationGraceMs >
  ...
  const mounts = value.readOnlyMounts.map((mount) => {
    ...
    for (const destination of destinations) {
      if (
        ...
        mount.destination.startsWith(`${destination}/`)
      ) {
...
async function requireSealedMounts(mounts: readonly SealedMount[]): Promise<void> {
  for (const mount of mounts) {
    ...
    if (
      information.isSymbolicLink() ||
      String(information.dev) !== mount.sourceDevice ||
      String(information.ino) !== mount.sourceInode ||
...
function ownerRecord(
  value: PrivateLinuxOwnerStateAllocationIdentity | PrivateLinuxSealedOwnerIdentity,
): OwnerRecordReference {
  if (value.kind === 'private-linux-owner-state-allocation/1') {
    return {
      allocationDigest: value.digest,
...
async function requireOwnerUnused(owner: PrivateLinuxSealedOwnerIdentity): Promise<void> {
  for (const path of [
    owner.runCgroup,
    join(owner.ownerStateDirectory, 'claim.json'),
...
async function requireOwnerRecord(reference: OwnerRecordReference): Promise<void> {
  const record = parseJsonLine(
    await readFile(join(reference.directory, 'owner.json'), 'utf8'),
    'rootless Linux owner record',
  )
  const expectedKeys =
    reference.ownerDigest === undefined
  ...
  if (
    record.kind !== 'private-linux-owner-state/1' ||
    record.allocationDigest !== reference.allocationDigest ||
    record.token !== reference.ownerToken ||
    record.ownerDigest !== reference.ownerDigest ||
    record.runCgroup !== reference.runCgroup ||
    record.mechanismDigest !== reference.mechanismDigest ||
    record.sealedPlanDigest !== reference.sealedPlanDigest ||
...
async function claimCancellation(
  reference: OwnerRecordReference,
): Promise<'won' | 'cancelled' | 'active'> {
  ...
  if (
    ...
    claim.token !== reference.ownerToken ||
    (claim.state !== 'active' && claim.state !== 'cancelled') ||
    Object.keys(claim).sort().join('\0') !==
      ['allocationDigest', 'kind', 'state', 'token'].join('\0')
  ) {
...
async function requireOwnerStateDirectory(owner: PrivateLinuxSealedOwnerIdentity): Promise<void> {
  await requireExactParent(
    owner.ownerStateParent,
    owner.ownerStateParentDevice,
    owner.ownerStateParentInode,
  )
  ...
  if (
    String(information.dev) !== owner.ownerStateDevice ||
    String(information.ino) !== owner.ownerStateInode
  ) {
    throw new Error('rootless Linux owner-state identity changed')
  }
}

async function requireAllocationParent(
  allocation: PrivateLinuxOwnerStateAllocationIdentity,
): Promise<void> {
...
async function requirePreparedCgroup(data: SealedOwnerData): Promise<void> {
  ...
  if (
    memory.trim() !== String(data.sealedPlan.limits.memoryBytes) ||
    pids.trim() !== String(data.sealedPlan.limits.pids) ||
    cpu.trim() !==
      `${data.sealedPlan.limits.cpuQuotaMicros} ${data.sealedPlan.limits.cpuPeriodMicros}` ||
    processes.trim() !== ''
  ) {
...
async function requirePayloadInCgroup(cgroup: string, payloadPid: number): Promise<void> {
  const members = new Set(
    (await readFile(join(cgroup, 'cgroup.procs'), 'utf8')).trim().split(/\s+/),
  )
  if (!members.has(String(payloadPid)))
    throw new Error('rootless Linux payload did not enter its Run cgroup')
}

function requirePreparedMessage(message: SupervisorPrepared, data: SealedOwnerData): void {
  if (
    message.runCgroup !== data.identity.runCgroup ||
    !safePositiveInteger(message.supervisorPid)
  ) {
...
function requireReadyMessage(
  ...
  if (
    message.runCgroup !== data.identity.runCgroup ||
    message.supervisorPid !== prepared.supervisorPid ||
...
function requireTerminalMessage(
  ...
  if (
    ...
    normalized.ownerDigest !== data.prepared.digest ||
    normalized.runCgroup !== data.identity.runCgroup ||
    normalized.supervisorPid !== prepared.supervisorPid
  ) {
    throw new Error('rootless supervisor returned an unexpected terminal')
  }
}

async function tryReadFinal(
  prepared: PrivateLinuxPreparedOwnerIdentity,
): Promise<FinalRecord | undefined> {
  try {
...
async function readFinal(prepared: PrivateLinuxPreparedOwnerIdentity): Promise<FinalRecord> {
  ...
  const final = normalizeFinal(
    JSON.parse(requireJsonLine(await readFile(path, 'utf8'), 'final receipt')),
  )
...
async function persistFinal(
  ...
  const temporary = join(
    directory,
    `.final-recovery-${process.pid}-${randomBytes(8).toString('hex')}`,
  )
  ...
  try {
    ...
    if (!sameJson(existing, final)) {
      throw new PrivateLinuxFenceUnconfirmedError(
        new Error('rootless cleanup owners produced conflicting receipts'),
      )
    }
  } finally {
...
function normalizeFinal(value: unknown): FinalRecord {
  ...
  if (record.type === 'recovered') {
    ...
    if (
      typeof exact.ownerDigest !== 'string' ||
      !DIGEST.test(exact.ownerDigest) ||
      typeof exact.runCgroup !== 'string' ||
      !canonicalCgroup(exact.runCgroup) ||
      typeof exact.recoveryBootId !== 'string' ||
      !BOOT_ID.test(exact.recoveryBootId) ||
      exact.stopReason !== 'recovered' ||
      exact.exitCode !== null ||
      exact.signal !== null ||
      exact.fenced !== true
    ) {
  ...
  if (
    ...
    typeof exact.runCgroup !== 'string' ||
    !canonicalCgroup(exact.runCgroup) ||
    !safePositiveInteger(exact.supervisorPid) ||
    !SUPERVISOR_STOP_REASONS.has(exact.stopReason as never) ||
    !(exact.exitCode === null || Number.isSafeInteger(exact.exitCode)) ||
    !(exact.signal === null || typeof exact.signal === 'string') ||
    typeof exact.fenced !== 'boolean' ||
...
function receiptFor(
  ...
  if (!final.fenced || (final.type === 'terminal' && final.cleanupError !== undefined)) {
    throw new PrivateLinuxFenceUnconfirmedError(
      new Error(
        final.type === 'terminal'
          ? (final.cleanupError ?? 'cleanup was not confirmed')
...
async function requireReleaseFiles(reference: ReleaseReference): Promise<void> {
  ...
  const claim = parseJsonLine(
    await readFile(join(reference.directory, 'claim.json'), 'utf8'),
    'owner claim',
  )
  if (
    (claim.state !== 'active' && claim.state !== 'cancelled') ||
  ...
  const expectation: ReleaseFileExpectation = {
    ...
    ...(reference.expectedOwnerDigest === undefined
      ? {}
      : { finalOwnerDigest: reference.expectedOwnerDigest }),
  }
  if (expectation.finalOwnerDigest !== undefined) {
    ...
    const rebootRecovery =
      expectation.claim === 'recovered' &&
      final.type === 'recovered' &&
      expectation.ownerBootId !== undefined &&
      final.recoveryBootId !== expectation.ownerBootId
    if (
      claim.state !== (expectation.claim === 'recovered' ? 'cancelled' : expectation.claim) &&
...
async function requireReleasableEntries(reference: ReleaseReference): Promise<void> {
  const allowed = new Set([
    ...
    'claim.json',
    'release.json',
    ...(reference.expectedOwnerDigest === undefined ? [] : ['final.json']),
  ])
  for (const name of await readdir(reference.directory)) {
...
async function* readJsonLines(stream: NodeJS.ReadableStream): AsyncIterable<unknown> {
  ...
  for await (const chunk of stream) {
    buffer += String(chunk)
    if (Buffer.byteLength(buffer) > CONTROL_BYTES)
      throw new Error('rootless supervisor control overflow')
...
function acceptOne(server: Server, timeoutMs: number): Promise<Socket> {
  return withTimeout(
    ...
    timeoutMs,
    'rootless supervisor connection',
  )
}
...
function childClose(child: ChildProcessWithoutNullStreams) {
  return new Promise<{ readonly code: number | null; readonly signal: string | null }>(
    (resolve, reject) => {
...
function requirePrivateDirectory(
  ...
  if (
    !information.isDirectory() ||
    information.isSymbolicLink() ||
    (Number(information.mode) & 0o077) !== 0
  ) {
...
function requireAbsolute(value: string, label: string): string {
  if (typeof value !== 'string' || !canonicalAbsolute(value))
    throw new TypeError(`${label} must be canonical absolute`)
  return value
}
...
function protectedHostSource(value: string): boolean {
  return (
    value === '/' ||
    ['/dev', '/proc', '/run', '/sys'].some((root) => value === root || value.startsWith(`${root}/`))
  )
}
...
export function privateLinuxResolverProjection(
  ...
  if (
    requestedSource !== '/etc/resolv.conf' ||
    destination !== '/etc/resolv.conf' ||
    !resolvedSource.startsWith('/run/')
  )
    return false
