...
export async function preparePrivateRootlessLinuxScope(
  ...
  if (!Number.isSafeInteger(uid) || uid === undefined || uid <= 0 || !Number.isSafeInteger(pid) || pid <= 0) {
  ...
  if (posix.basename(scope) !== unit || posix.dirname(scope) === CGROUP_ROOT ||
  ...
  await dependencies.requireAccess(`${scope}/cgroup.subtree_control`, constants.R_OK | constants.W_OK);
  ...
  for (let round = 0; round < MAX_MOVE_ROUNDS; round += 1) {
    ...
    const ordered = [...processes.filter((candidate) => candidate !== pid),
...
function parseMarker(environment: NodeJS.ProcessEnv): DelegationMarker | undefined {
  ...
  if (unit === undefined || socketPath === undefined || token === undefined || !UNIT.test(unit) ||
      !TOKEN.test(token) || !READY_SOCKET.test(socketPath)) {
...
function requirePreparedObservation(
  ...
  if (!delegated.startsWith(`${CGROUP_ROOT}/`) || posix.normalize(delegated) !== delegated ||
...
function requireCommand(command: readonly [string, ...string[]]): readonly [string, ...string[]] {
  if (command.length === 0 || !isAbsolute(command[0]) || command.some((value) => value.includes("\0"))) {
...
async function resolveManager(): Promise<string> {
  for (const candidate of MANAGER_CANDIDATES) {
    try {
      ...
      if (isAbsolute(resolved) && information.isFile() && (information.mode & 0o111) !== 0 &&
...
export async function reexecutePrivateRootlessLinuxCommand(
  ...
  let completion: Promise<{ readonly exitCode: number | null; readonly signal: NodeJS.Signals | null }> | undefined;
  ...
  try {
    ...
    const launched = spawn(managerPath, [
      "--user",
      "--scope",
      "--collect",
      "--quiet",
...
function waitForStartup(
  ...
  return new Promise((resolve) => {
    ...
    void completion.then(() => finish("exited"), () => finish("exited"));
  });
}
...
async function startLifetimeTimer(
  ...
  await executeControl(managerPath, [
    "--user",
    "--collect",
    "--quiet",
...
async function stopLifetimeTimer(
  ...
  for (const suffix of [".timer", ".service"] as const) {
    await executeControl(controlPath, ["--user", "stop", `${lifetimeTimer}${suffix}`], environment)
      .catch(() => undefined);
  }
  await Promise.all([
...
async function killTransientScope(
  ...
  await executeControl(controlPath, [
    "--user",
    "--wait",
    "kill",
    "--kill-whom=all",
...
async function observeControlGroup(
  ...
  for (;;) {
    ...
    if (state.controlGroup !== "") {
      ...
      if (!relative.startsWith("/") || relative.endsWith("/") || relative.includes("\0") ||
          relative.includes("\\") || posix.normalize(relative) !== relative ||
      ...
      if (await realpath(path) !== path) throw new Error("the transient scope path is not canonical");
      return path;
    }
...
async function requireUnitCollected(
  ...
  for (;;) {
    ...
    if (Date.now() >= deadline) throw new Error(`transient user-manager unit was not collected: ${unit}`);
    await delay(20);
  }
}
...
async function unitProperties(
  ...
  unit: string,
  environment: NodeJS.ProcessEnv,
): Promise<{ readonly loadState: string; readonly activeState: string; readonly controlGroup: string }> {
  const result = await executeControl(controlPath, [
    "--user",
    "show",
...
async function settleChild(
  ...
  await Promise.race([
    ...
    delay(COLLECTION_TIMEOUT_MS).then(() => { throw new Error("the scope manager process did not settle"); }),
  ]);
}
...
function readinessServer(socketPath: string, token: string): {
...
async function acknowledgeReady(marker: DelegationMarker, delegatedCgroup: string): Promise<void> {
  await new Promise<void>((resolve, reject) => {
    ...
    const timeout = setTimeout(() => socket.destroy(new Error("rootless acquisition acknowledgement timed out")),
      READY_TIMEOUT_MS);
    socket.setEncoding("utf8");
