...
export type PrivateRootRunTerminal =
  | RunHostTerminal
  | {
...
export function createPrivateRootRunRequest(input: {
  ...
  if (!Number.isSafeInteger(input.deadlineUnixMs) || input.deadlineUnixMs < 0) {
    invalid(
      ...
      'root Run deadline must be a non-negative safe Unix millisecond value',
    )
  }
...
export function privateRootRunOriginDigest(value: PrivateRootRunOrigin): string {
  ...
  return privateDomainDigest('JIG-Private-Root-Origin-External-Submission/1', {
    submissionId: origin.submissionId,
  })
}

export function normalizePrivateRootRunIdentityInput(value: unknown): PrivateRootRunIdentityInput {
  ...
  return Object.freeze({
    ...
    coordinatorEpoch: positiveSafeInteger(
      ...
      'root Run identity coordinator epoch',
    ),
  })
}
...
export function requirePrivateRootSubmissionId(value: unknown): asserts value is string {
  ...
  if (length < 1 || length > 1024) {
    invalid(
      ...
      'root submission ID must contain 1 to 1024 Unicode scalar values',
    )
  }
}
...
export function decodePrivateRootRunRequest(bytes: Uint8Array): PrivateRootRunRequest {
  ...
  if (value.kind !== 'private-root-run-request/1')
    throw new TypeError('root Run request kind is invalid')
...
export function privateRootRequestDigest(request: PrivateRootRunRequest): string {
  return privateDomainDigest('JIG-Private-Root-Request/1', request as unknown as JsonValue)
}
...
export function normalizePrivateRootSpawnIntent(value: unknown): PrivateRootRunSpawnIntent {
  ...
  if (root.kind !== 'private-root-spawn-intent/1')
    throw new TypeError('root Run spawn intent kind is invalid')
  const candidateRevision = positiveSafeInteger(
    root.candidateRevision,
    'root Run candidate revision',
  )
  const coordinatorEpoch = positiveSafeInteger(root.coordinatorEpoch, 'root Run coordinator epoch')
  const deadlineUnixMs = nonnegativeSafeInteger(
    root.deadlineUnixMs,
    'root Run spawn intent deadline',
  )
  return Object.freeze({
...
export function normalizePrivateRootTerminal(value: unknown): PrivateRootRunTerminal {
  ...
  if (status === 'succeeded') {
    const root = exactRecord(
      ...
      'successful root Run terminal',
    )
    const result = exactRecord(root.result, ['outcome', 'output'], 'root Run result')
    if (typeof result.outcome !== 'string' || result.outcome.length === 0)
      throw new TypeError('root Run outcome is invalid')
    return Object.freeze({
      status: 'succeeded',
  ...
  if (status === 'failed') {
    ...
    const root = exactRecord(
      value,
      hasDetails
        ? ['status', 'code', 'message', 'details', 'diagnostics']
        : ['status', 'code', 'message', 'diagnostics'],
      'failed root Run terminal',
    )
    if (
      typeof root.code !== 'string' ||
      !RUN_HOST_FAILURE_CODES.has(root.code) ||
...
function normalizeDiagnostics(
  value: unknown,
): Extract<RunHostTerminal, { readonly status: 'succeeded' }>['diagnostics'] {
  ...
  if (
    ...
    !Number.isSafeInteger(diagnostics.stderrBytes) ||
    (diagnostics.stderrBytes as number) < 0 ||
