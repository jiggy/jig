...
export const PRIVATE_OPENAI_APIS = Object.freeze(['responses', 'chat-completions'] as const)

export type PrivateOpenAIApi = (typeof PRIVATE_OPENAI_APIS)[number]
...
export type PrivateOpenAIAgentErrorCode = (typeof PRIVATE_OPENAI_AGENT_ERROR_CODES)[number]
...
export function encodePrivateOpenAIAgentRequest(value: unknown): Uint8Array {
...
export function decodePrivateOpenAIAgentRequest(bytes: Uint8Array): PrivateOpenAIAgentRequest {
  if (bytes.byteLength === 0 || bytes.byteLength > PRIVATE_OPENAI_AGENT_REQUEST_BYTES) {
...
export function encodePrivateOpenAIAgentSuccess(value: unknown): Uint8Array {
...
export function decodePrivateOpenAIAgentResponse(
  ...
  if (bytes.byteLength === 0 || bytes.byteLength > PRIVATE_OPENAI_AGENT_RESPONSE_BYTES) {
  ...
  if (
    !exactKeys(record, ['code', 'message', 'protocol', 'status']) ||
...
function requirePrivateOpenAIAgentRequest(value: unknown): PrivateOpenAIAgentRequest {
  const record = ordinaryRecord(value)
...
function validApi(value: unknown): value is PrivateOpenAIApi {
  return typeof value === 'string' && (PRIVATE_OPENAI_APIS as readonly string[]).includes(value)
}
