...
export function createPrivateActivationRecipeObservation(
  ...
  const valueWithoutDigest = Object.freeze({
    ...
    wouldGrantAuthorityDigest: requireDigest(root.wouldGrantAuthorityDigest, "would-grant authority"),
...
function normalizeEntry(input: unknown, index: number): PrivateActivationPlanningEntry {
  ...
  const state = readDataField(root.disposition, "state", `activation disposition ${targetKey(target)}`);
  ...
  if (state === "planned") {
    ...
    if (observation.requestDigest !== requestDigest) {
      throw new TypeError(`recipe observation does not belong to activation request ${targetKey(target)}`);
    }
...
function normalizeTarget(input: unknown): RunTargetIdentity {
  ...
  if (kind === "flow") {
    ...
    return Object.freeze({ kind: "flow" as const, path: normalizeProjectPath(value.path, "Flow target") });
  }
  if (kind === "binding") {
...
function readClosedRecord(value: unknown, fields: readonly string[], label: string): Record<string, unknown> {
