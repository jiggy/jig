...
export async function preparePrivateBunPackage(input: {
  ...
  try {
    ...
    fact = await requireFact(replacePrivateBunPreparationOwner({
      projectRoot: input.projectRoot,
      coordinator: input.coordinator,
    ...
    try {
      ...
      throw new AggregateError([error, cleanup], "Bun preparation and cleanup did not both complete");
    }
    throw error;
  }
}
  ...
  try {
    ...
    for await (const value of jsonLines(component.stdout, MAX_WORKER_LINE_BYTES)) {
      ...
      if (message.type === "prepared") {
        ...
        if (terminal !== undefined || typeof message.code !== "string" ||
            !WORKER_FAILURE_CODES.has(message.code) || typeof message.message !== "string") {
    ...
    if (terminal === undefined ||
        (!(terminal instanceof CheckError) && fence.exitCode !== 0) || !fence.fenced) {
...
function normalizeCheckpoint(value: JsonValue): PreparationCheckpoint {
  ...
  if (record.kind !== "private-bun-preparation-owner/1") throw new TypeError("Bun preparation owner is invalid");
  ...
  if (owner.ownerStateAllocationDigest !== allocation.digest ||
      owner.ownerStateParent !== allocation.parent || owner.ownerStateName !== allocation.name ||
      owner.ownerStateDirectory !== allocation.directory || owner.ownerToken !== allocation.ownerToken) {
...
async function preparationOwnerParent(projectRoot: string): Promise<string> {
  ...
  if (!information.isDirectory() || information.isSymbolicLink() || information.uid !== uid ||
      (information.mode & 0o077) !== 0 || await realpath(parent) !== parent) {
...
async function capturedFromPrepared(value: readonly unknown[]): Promise<CapturedPackage> {
  ...
  for (const raw of value) {
    ...
    if (typeof record.path !== "string" || typeof record.content !== "string" ||
        Reflect.ownKeys(record).length !== 2) throw protocolFailure();
  ...
  const backing: CapturedPackageBacking = {
    ...
    async dispose(): Promise<void> { contents.clear(); },
  };
