...
export async function openPrivateProjectSession(input: {
  ...
  try {
    ...
    try { await roots?.dispose(); } catch (cleanup) { failures.push(cleanup); }
...
function createSession(
  ...
  const session: ProjectSession = Object.freeze({
    ...
    async apply(value: ProjectApplyRequest): Promise<ProjectApplyReceipt> {
      ...
      try {
        ...
        await owner.verify();
        return Object.freeze("admission" in receipt
          ? {
  ...
  function enterOperation(): () => void {
    ...
    let settle!: () => void;
    const token = new Promise<void>((resolve) => { settle = resolve; });
  ...
  async function finishClose(): Promise<void> {
    ...
    try { await rootClosure; } catch (error) { failures.push(error); }
  ...
  function handleOperationError(error: unknown, operation: "plan" | "apply"): ProjectAdministrationError {
...
async function preparePackageStore(owner: PrivateProjectSessionOwner): Promise<string> {
  ...
  try {
    ...
    try { await mkdir(storePath, { mode: 0o700 }); }
    ...
    try {
      ...
      if (observed.dev !== actual.dev || observed.ino !== actual.ino) {
        throw new ProjectAdministrationError("PROJECT_UNSAFE", "project package store changed while opening");
      }
    } finally {
      await store.close();
    }
...
function requireProtectedStore(information: BigIntStats, projectDevice: bigint): void {
  if (!information.isDirectory() || information.isSymbolicLink() ||
      information.dev !== projectDevice || information.uid !== BigInt(currentEuid()) ||
...
export function projectError(
  ...
  if (error instanceof CheckError) {
    if (error.code === "COORDINATOR_BUSY" || error.code === "ADMISSION_STATE_BUSY" ||
        error.code === "PROJECT_BUSY" || error.code === "PROJECT_SOURCE_CHANGED") {
      return new ProjectAdministrationError("PROJECT_BUSY", "project changed or is busy; retry the operation");
    }
    if (error.code === "LOCK_MISMATCH") {
      return new ProjectAdministrationError("LOCK_MISMATCH", "visible jig.lock does not match the proposed lock");
    }
    if (error.code === "ADMISSION_PLAN_MISSING") {
      return new ProjectAdministrationError("PLAN_NOT_FOUND", "reviewed project Plan does not exist");
    }
    ...
    if (operation === "acquire" && (
      error.code.startsWith("PROJECT_ROOT") || error.code.startsWith("ADMISSION_STATE") ||
...
function currentEuid(): number {
  if (typeof process.geteuid !== "function") {
    throw new ProjectAdministrationError("UNAVAILABLE", "project ownership requires a Unix user identity");
  }
