...
const SDK_ENTRY = process.env.JIG_EVALUATOR_SDK ?? '/jig-evaluator/project-evaluator-sdk.bundle.js'
...
async function readRequest(): Promise<WorkerRequest> {
  ...
  for (const candidate of value.modules) {
    if (
      ...
      paths.has(candidate.projectPath)
    ) {
    ...
    for (const edge of candidate.imports) {
      if (
        ...
        typeof edge.projectPath !== 'string' ||
  ...
  for (const module of modules) {
    for (const edge of module.imports) {
      if (!paths.has(edge.projectPath)) {
        throw tagged(
          'PROJECT_EVALUATOR_PROTOCOL',
          'evaluator import target is absent from its module closure',
        )
      }
    }
  }
      ...
      plugins: [
        {
          ...
          setup(builder): void {
            ...
            builder.onResolve({ filter: /^@jigging\/jig$/ }, (arguments_) => {
              if (arguments_.kind !== 'import-statement' || !modules.has(arguments_.importer)) {
                throw deniedImport(arguments_.path)
              }
            ...
            builder.onResolve({ filter: /.*/ }, (arguments_) => {
              if (arguments_.kind !== 'import-statement') {
                throw deniedImport(
                  `${arguments_.path} (${arguments_.kind} from ${arguments_.importer})`,
                )
              }
...
function evaluate(code: string): JsonValue {
  ...
  if (!isRecord(exports) || Object.keys(exports).length !== 1 || !('default' in exports)) {
    throw tagged(
      'PROJECT_DEFAULT_EXPORT',
      'compiled author module did not expose one default value',
    )
  }
  ...
  return JSON.parse(
    new TextDecoder().decode(canonicalJson(exports.default as JsonValue)),
  ) as JsonValue
}
...
function errorCode(error: unknown): string {
  ...
  if (
    ...
    EVALUATION_CODES.has(error.code)
  ) {
    return error.code
  }
