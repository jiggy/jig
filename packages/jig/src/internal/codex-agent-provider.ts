 ...
 */
export async function projectPrivateCodexSubscriptionCredential(path: string): Promise<Uint8Array> {
  ...
  if (
    !isRecord(parsed) ||
    !exactKeys(parsed, ['OPENAI_API_KEY', 'auth_mode', 'last_refresh', 'tokens']) ||
    parsed.auth_mode !== 'chatgpt' ||
    parsed.OPENAI_API_KEY !== null ||
    !isRecord(parsed.tokens) ||
    !exactKeys(parsed.tokens, ['access_token', 'account_id', 'id_token', 'refresh_token'])
  ) {
    throw new Error('Codex subscription credential is invalid')
  }
...
function requireSubscriptionCredential(value: Uint8Array): Uint8Array {
  ...
  if (
    ...
    !Number.isFinite(Date.parse(parsed.last_refresh)) ||
    !isRecord(parsed.tokens) ||
    !exactKeys(parsed.tokens, ['access_token', 'account_id', 'id_token', 'refresh_token'])
  ) {
    throw new Error('Codex subscription credential is invalid')
  ...
  if (
    ...
    encoder.encode(accessToken).byteLength > MAX_SECRET_BYTES ||
    idToken !== accessToken ||
    refreshToken !== '' ||
    typeof accountId !== 'string' ||
    accountId.length === 0 ||
...
async function exactFile(path: string, label: string): Promise<string> {
  const exact = await realpath(path)
  const information = await lstat(exact)
