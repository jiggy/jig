...
export function createPrivateActivationCandidateV5(
  ...
  const activationMeaningDigest = computeActivationMeaningDigest(observedSemanticDigest, targets)
...
export function requirePrivateCreatedActivationCandidateV5(
  ...
  if (value === null || typeof value !== 'object' || !createdCandidatesV5.has(value)) {
    throw new TypeError(
      'activation candidate/5 was not built from a retained project and resolution',
    )
  }
...
export function createPrivateActivationPlanV2(input: {
  ...
  const source = exactObject(
    input,
    ['candidate', 'candidateRevision', 'baseGeneration', 'lockMode', 'observedLock', 'operation'],
  ...
  if (observedState === 'absent') {
    ...
    const lock = decodePrivateProjectLocalLock(
      encodePrivateProjectLocalLock(present.lock as PrivateProjectLocalLock),
    )
...
function normalizeCandidateV5(
  ...
  const expectedResolutionInput = privateDomainDigest('JIG-Package-Project-Resolution-Input/1', {
    captureDigest,
    planningObservationDigest,
  })
...
function normalizeCandidateTargets(
  ...
  for (let index = 1; index < targets.length; index += 1) {
    if (
      privateActivationTargetKey(targets[index - 1]!.request.target) ===
  ...
  requireExactTargetSet(
    targets.map((target) => target.request.target),
    lock,
  )
...
function normalizePlanV2(input: unknown, decodedLocks = false): PrivateActivationPlanV2 {
  ...
  if (
    resolutionInputDigest !==
    privateDomainDigest('JIG-Package-Project-Resolution-Input/1', {
      captureDigest,
      planningObservationDigest,
    })
  ) {
  ...
  const baseGeneration =
    root.baseGeneration === null ? null : requireDigest(root.baseGeneration, 'base generation')
  if (root.operation === 'lock-repair') {
    ...
    if (observedLock.state === 'present' && samePrivateLocks(observedLock.lock, proposedLock)) {
      throw new TypeError(
        'activation plan/2 lock repair requires an absent or drifted visible lock',
      )
    }
  }
...
function normalizeAdmission(input: unknown): PrivateActivationAdmission {
  const root = exactObject(
    input,
    ['kind', 'baseGeneration', 'planDigest', 'candidateRevision', 'candidateDigest', 'lockDigest'],
...
function requireRequestLockProjection(
  ...
  const selected =
    request.target.kind === 'binding'
      ? (lock.bindings[request.target.id]?.slots ?? {})
      : (packageValue.slots ?? {})
  ...
  if (Object.keys(request.settings).length !== 0 || request.boundAttachments !== undefined) {
...
function requireExactTargetSet(
  ...
  const keys = [
    ...
    ...Object.keys(lock.bindings).map((id) => privateActivationTargetKey({ kind: 'binding', id })),
  ].sort()
...
function normalizeEmbeddedLock(
  ...
  if (!decoded) {
    ...
    return decodePrivateProjectLocalLock(
      encodePrivateProjectLocalLock(value as PrivateProjectLocalLock),
    )
  }
...
function samePrivateLocks(left: PrivateProjectLocalLock, right: PrivateProjectLocalLock): boolean {
  return sameBytes(encodePrivateProjectLocalLock(left), encodePrivateProjectLocalLock(right))
}
