  ...
  for (const root of roots) {
    ...
    try {
      ...
      for (const name of fingerprint.selectedNames) {
        ...
        const member = await openSelectedMember(directory.handle, name, projectPath, fingerprint)
        try {
    ...
    observations.map((observation) =>
      Object.freeze({
        ...
        members: Object.freeze(
          (observation.fingerprint?.selectedNames ?? []).map(
...
async function captureExact(
  ...
  for (const observation of observations) {
    ...
    if (current === undefined)
      sourceChanged('exact Flow member disappeared during capture', observation.path)
    try {
...
async function openChildDirectory(
  ...
  if (!observed.isDirectory())
    invalid('PROJECT_MEMBER_KIND', 'Flow source member must be a directory', logicalPath)
  ...
  try {
    ...
    unavailable(
      'PROJECT_SOURCE_IO',
      `cannot open project directory: ${errorText(error)}`,
      logicalPath,
    )
  }
  try {
...
async function openSelectedMember(
  ...
  try {
    ...
    if (
      ...
      ['PROJECT_MEMBER_MISSING', 'PROJECT_MEMBER_KIND', 'PROJECT_SOURCE_SYMLINK'].includes(
        error.code,
      )
    ) {
...
async function fingerprintDiscoveryRoot(directory: OpenDirectory): Promise<DiscoveryFingerprint> {
  ...
  for await (const name of readDirectoryNames(directory.handle)) {
    ...
    try {
      ...
      unavailable(
        'PROJECT_SOURCE_IO',
        `cannot inspect Flow discovery entry: ${errorText(error)}`,
        name,
      )
    }
...
async function verifyDiscoveryObservation(
  ...
  if (current === undefined)
    sourceChanged('discovery root disappeared during capture', observation.root)
  try {
...
async function reopenObservedDirectory(
  ...
  try {
    ...
    if (
      error instanceof CheckError &&
      ['PROJECT_MEMBER_KIND', 'PROJECT_SOURCE_SYMLINK'].includes(error.code)
    ) {
...
function validateSource(source?: ProjectSource): ProjectSource | undefined {
  ...
  if (source.kind === 'discover') {
    ...
    for (const root of roots) {
      if (typeof root !== 'string')
        invalid('PROJECT_SOURCE', 'Flow discovery roots must be strings')
      validateProjectSourcePath(root)
      if (GLOB_CHARACTERS.test(root))
        invalid('PROJECT_SOURCE', 'Flow discovery roots cannot contain glob characters', root)
    }
  ...
  if (source.kind === 'members') {
    if (!Array.isArray(source.paths))
      invalid('PROJECT_SOURCE', 'Flow exact source requires a path array')
...
function sameDiscoveryFingerprint(
...
function directoryEntryName(entry: unknown): Uint8Array {
  ...
  unavailable(
    ...
    'runtime cannot enumerate raw Flow source names incrementally',
  )
}
