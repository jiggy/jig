...
function createCapture(projectPath: string, source: Uint8Array): CapturedAuthorModule {
  ...
  const capture = Object.freeze({
    ...
    read(): Uint8Array {
      if (disposed)
        unavailable(
          'PROJECT_CAPTURE_CLOSED',
          'author-module capture has been disposed',
          projectPath,
        )
      return bytes.slice()
    },
...
function createClosure(
  ...
  const identity = {
    ...
    modules: modules.map(({ projectPath, sourceBytes, sourceDigest, imports: edges }) => ({
      ...
      sourceBytes,
      sourceDigest,
      imports: edges.map(({ specifier, projectPath: target }) => ({
        specifier,
        projectPath: target,
      })),
    })),
  ...
  const closure = Object.freeze({
    ...
    read(projectPath: string): Uint8Array {
      if (disposed)
        unavailable('PROJECT_CAPTURE_CLOSED', 'author closure has been disposed', projectPath)
      const bytes = sources.get(projectPath)
      if (bytes === undefined)
        invalid('PROJECT_EVALUATOR_SOURCE', 'module is outside the author closure', projectPath)
      return bytes.slice()
    },
    dispose(): void {
      if (disposed) return
...
function scanStaticImports(importer: string, source: string): readonly CapturedAuthorImport[] {
  const BunRuntime = (
  ...
  for (const item of scanned) {
    ...
    edges.set(
      `${item.path}\0${target}`,
      Object.freeze({ specifier: item.path, projectPath: target }),
    )
  }
  return Object.freeze(
    [...edges.values()].sort((left, right) => {
      ...
      return specifierOrder === 0
        ? compareProjectPaths(left.projectPath, right.projectPath)
        : specifierOrder
    }),
  )
}
...
async function openProjectFile(
  ...
  try {
    ...
    await closeAfterFailure(
      result === undefined ? [] : [result.handle],
      error,
      'author-module descriptor',
    )
  }
...
async function openEntry(
  ...
  try {
    ...
    if (isEntryRace(error)) {
      if (missingIsChange)
        sourceChanged('author module path changed before it was reopened', logicalPath)
      invalid('PROJECT_EVALUATOR_SOURCE', 'author module path is missing', logicalPath)
    }
    unavailable(
      'PROJECT_SOURCE_IO',
      `cannot inspect author module path: ${errorText(error)}`,
      logicalPath,
    )
  }
  ...
  try {
    ...
    unavailable(
      'PROJECT_SOURCE_IO',
      `cannot open author module path: ${errorText(error)}`,
      logicalPath,
    )
  }
...
function validateAuthorPath(projectPath: string): void {
  ...
  if (isProtectedProjectPath(projectPath)) {
    invalid(
      ...
      'author module cannot use protected .jig state',
      projectPath,
    )
  }
