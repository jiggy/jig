...
export async function captureDeclarationSource(
  ...
  if (normalized.kind === 'discover') {
    ...
    observations.push(
      Object.freeze({ kind: 'members' as const, members: Object.freeze([...normalized.paths]) }),
    )
  }
...
function validateSource(source?: ProjectSource): ProjectSource | undefined {
  ...
  if (source.kind === 'discover') {
    if (!Array.isArray(source.roots) || source.roots.length === 0)
      invalid('PROJECT_SOURCE', 'declaration discovery requires roots')
    const roots = [...source.roots]
    for (const root of roots) {
      validateSourcePath(root)
      if (GLOB_CHARACTERS.test(root))
        invalid('PROJECT_SOURCE', 'declaration roots cannot contain glob characters', root)
    }
...
async function fingerprintRoot(directory: OpenDirectory, root: string): Promise<Fingerprint> {
  ...
  for await (const name of readDirectoryNames(directory.handle)) {
    ...
    try {
      ...
      unavailable(
        'PROJECT_SOURCE_IO',
        `cannot inspect declaration entry: ${errorText(error)}`,
        `${root}/${name}`,
      )
    }
    ...
    if (selectedNames.length > MAX_MEMBERS)
      invalid('PROJECT_SOURCE_LIMIT', `declaration source exceeds ${MAX_MEMBERS} members`, root)
  }
...
function requireDeclarationFile(information: BigIntStats, path: string): void {
  if (!information.isFile())
    invalid('PROJECT_MEMBER_KIND', 'declaration member must be a regular file', path)
  if (information.nlink !== 1n)
    invalid('PROJECT_MEMBER_COLLISION', 'declaration member must be singly linked', path)
}

function assertMembers(members: readonly DeclarationMember[]): void {
  if (members.length > MAX_MEMBERS)
    invalid('PROJECT_SOURCE_LIMIT', `declaration source exceeds ${MAX_MEMBERS} members`)
  ...
  for (const value of members) {
    ...
    if (prior !== undefined)
      invalid(
        ...
        `declaration ID ${value.id} is provided by ${prior} and ${value.projectPath}`,
      )
...
function assertPaths(paths: readonly string[]): void {
  ...
  for (let index = 1; index < paths.length; index += 1) {
    if (paths[index - 1] === paths[index])
      invalid('PROJECT_SOURCE_COLLISION', `project source repeats ${paths[index]}`)
  }
}
...
async function openEntry(
  ...
  try {
    ...
    unavailable(
      'PROJECT_SOURCE_IO',
      `cannot inspect project path: ${errorText(error)}`,
      logicalPath,
    )
  }
  if (observed.isSymbolicLink())
    invalid('PROJECT_SOURCE_SYMLINK', 'project source paths cannot contain symlinks', logicalPath)
  if (directory ? !observed.isDirectory() : !observed.isFile()) {
    invalid(
      'PROJECT_MEMBER_KIND',
      directory
        ? 'project source parent must be a directory'
        : 'declaration member must be a regular file',
      logicalPath,
    )
  }
  ...
  if (
    !sameIdentity(observed, information) ||
...
async function* readDirectoryNames(directory: FileHandle): AsyncIterable<string> {
  ...
  try {
    while (true) {
      ...
      if (count > MAX_ROOT_ENTRIES || bytes > MAX_ROOT_NAME_BYTES)
        unavailable('PROJECT_SOURCE_LIMIT', 'declaration root exceeds its enumeration budget')
      try {
...
function directoryEntryName(entry: unknown): Uint8Array {
  ...
  if (
    ...
    entry !== null &&
    'name' in entry &&
    (entry as { name: unknown }).name instanceof Uint8Array
  ) {
    return (entry as { name: Uint8Array }).name
  }
  unavailable(
    'PROJECT_SOURCE_UNAVAILABLE',
    'runtime cannot enumerate raw declaration names incrementally',
  )
}
