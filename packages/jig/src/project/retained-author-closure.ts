...
export async function retainAuthorClosure(
  ...
  const files = Object.freeze(closure.modules.map(({ projectPath, sourceBytes }) => Object.freeze({
    path: projectPath,
    size: sourceBytes,
  })));
  const bytes = new Map(closure.modules.map(({ projectPath }) => [projectPath, closure.read(projectPath)]));
  let disposed = false;
  const backing: CapturedPackageBacking = {
    async *stream(path: string, maximumBytes?: number): AsyncIterable<Uint8Array> {
      ...
      const end = maximumBytes === undefined ? value.byteLength : Math.min(value.byteLength, maximumBytes);
