...
export function buildPrivateActivationRequests(
  ...
  if (requests.length > PRIVATE_ACTIVATION_TARGET_LIMIT) {
    throw new TypeError(`activation requests exceed ${PRIVATE_ACTIVATION_TARGET_LIMIT} targets`)
  }
...
export function restorePrivateActivationRequest(value: unknown): PrivateActivationRequest {
  ...
  const entrypointValue = exactObject(
    root.entrypoint,
    root.entrypoint !== null &&
      typeof root.entrypoint === 'object' &&
...
export function resolveLinkedPackageProjectObservation(
  ...
  const plannedByTarget = new Map(
    snapshot.entries.map((entry) => [privateActivationTargetKey(entry.target), entry]),
  )
  ...
  const resolutionInputDigest = privateDomainDigest('JIG-Package-Project-Resolution-Input/1', {
    captureDigest,
  ...
  return Object.freeze({
    ...
    captureDigest,
    semanticDigest: privateDomainDigest('JIG-Package-Project-Observed-Semantics/1', semanticValue),
...
function captureJsonValue(value: unknown, label: string, active: WeakSet<object>): JsonValue {
  if (
    value === null ||
    typeof value === 'string' ||
