...
export async function openPrivateProjectRoot(projectRoot: string): Promise<PrivateProjectRoot> {
  ...
  try {
    ...
    unavailable(
      'PROJECT_ROOT_IO',
      `cannot inspect project root: ${errorText(error)}`,
      requestedPath,
    )
  }
  if (observed.isSymbolicLink())
    invalid('PROJECT_ROOT', 'project root must not be a symlink', requestedPath)
  if (!observed.isDirectory())
    invalid('PROJECT_ROOT', 'project root is not a directory', requestedPath)
  ...
  try {
    ...
    unavailable(
      'PROJECT_ROOT_IO',
      `cannot inspect opened project root: ${errorText(error)}`,
      requestedPath,
    )
  }
  ...
  const root = Object.freeze({
    ...
    async verify(): Promise<void> {
      if (disposed)
        unavailable('PROJECT_ROOT_CLOSED', 'project-root capture has been disposed', requestedPath)
    ...
    async dispose(): Promise<void> {
      ...
      try {
        ...
        unavailable(
          'PROJECT_ROOT_IO',
          `cannot close project root: ${errorText(error)}`,
          requestedPath,
        )
      }
    },
