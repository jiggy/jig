...
const KEYWORDS = new Set([
  ...
  "dependentRequired", "dependentSchemas", "prefixItems", "items", "contains",
...
class GraphBuilder {
  ...
  addEmbeddedRoots(entries: readonly EmbeddedSchemaSource[], rootDefs?: JsonObject): void {
    ...
    for (const entry of entries) {
      ...
      if (entry.pointer === "/$defs" || entry.pointer.startsWith("/$defs/")) {
        this.fail("SCHEMA_INVALID", "an embedded schema root overlaps descriptor $defs", entry.pointer);
      }
  ...
  private visit(value: JsonValue, pointer: string, depth: number, allowDefinitions: boolean): void {
    if (depth > SCHEMA_1_LIMITS.depth) {
      this.fail("SCHEMA_LIMIT_EXCEEDED", `schema nesting exceeds depth ${SCHEMA_1_LIMITS.depth}`, pointer);
    }
    if (typeof value !== "boolean" && !isObject(value)) {
      this.fail("SCHEMA_INVALID", "a schema-valued position must contain an object or boolean", pointer);
    }
    ...
    if (this.nodeCount > SCHEMA_1_LIMITS.nodes) {
      this.fail("SCHEMA_LIMIT_EXCEEDED", `schema graph exceeds ${SCHEMA_1_LIMITS.nodes} nodes`, pointer);
    }
      ...
      switch (keyword) {
        case "$schema":
          if (pointer !== "" || !allowDefinitions || value !== SCHEMA_1_URI) {
            this.fail("SCHEMA_INVALID", "$schema is allowed only as the exact file-root declaration", keywordPointer, keyword);
          }
          break;
        case "$defs": {
          if (!allowDefinitions) {
            this.fail("SCHEMA_INVALID", "$defs is allowed only on a Schema/1 file root", keywordPointer, keyword);
          }
        ...
        case "enum":
          ...
          break;
        case "const":
          if (!isScalar(value)) this.fail("SCHEMA_INVALID", "const is limited to a JSON/1 scalar", keywordPointer, keyword);
          break;
        case "allOf":
        case "anyOf":
        case "oneOf": {
          const schemas = this.arrayValue(value, keywordPointer, keyword);
          if (schemas.length === 0) this.fail("SCHEMA_INVALID", `${keyword} must not be empty`, keywordPointer, keyword);
          this.visitSchemaArray(schemas, keywordPointer, depth);
          break;
        }
        case "prefixItems": {
          const schemas = this.arrayValue(value, keywordPointer, keyword);
          if (schemas.length === 0) this.fail("SCHEMA_INVALID", "prefixItems must not be empty", keywordPointer, keyword);
          this.visitSchemaArray(schemas, keywordPointer, depth);
          break;
        }
        case "not":
        ...
        case "exclusiveMaximum":
          if (typeof value !== "number") this.fail("SCHEMA_INVALID", `${keyword} must be a number`, keywordPointer, keyword);
          break;
      }
    }
  }
  ...
  private validateType(value: JsonValue, pointer: string): void {
    if (typeof value === "string") {
      if (!SIMPLE_TYPES.has(value)) this.fail("SCHEMA_INVALID", `unknown type ${value}`, pointer, "type");
      return;
    }
    const values = this.arrayValue(value, pointer, "type");
    if (values.length === 0) this.fail("SCHEMA_INVALID", "type array must not be empty", pointer, "type");
    ...
    for (const item of values) {
      ...
      if (seen.has(item)) this.fail("SCHEMA_INVALID", "type array contains a duplicate", pointer, "type");
      seen.add(item);
    }
  }
  ...
  private validateEnum(value: JsonValue, pointer: string): void {
    ...
    for (const item of values) {
      if (!isScalar(item)) this.fail("SCHEMA_INVALID", "enum is limited to JSON/1 scalars", pointer, "enum");
    }
  }

  private stringValue(value: JsonValue, pointer: string, keyword: string): string {
    if (typeof value !== "string") this.fail("SCHEMA_INVALID", `${keyword} must be a string`, pointer, keyword);
    return value;
  }

  private arrayValue(value: JsonValue, pointer: string, keyword: string): readonly JsonValue[] {
    if (!Array.isArray(value)) this.fail("SCHEMA_INVALID", `${keyword} must be an array`, pointer, keyword);
    return value;
  }

  private objectValue(value: JsonValue, pointer: string, keyword: string): JsonObject {
    if (!isObject(value)) this.fail("SCHEMA_INVALID", `${keyword} must be an object`, pointer, keyword);
    return value;
  }
  ...
  private stringArray(value: JsonValue, pointer: string, keyword: string): readonly string[] {
    ...
    const seen = new Set<string>();
    for (const item of values) {
      if (typeof item !== "string") this.fail("SCHEMA_INVALID", `${keyword} must contain only strings`, pointer, keyword);
      if (seen.has(item)) this.fail("SCHEMA_INVALID", `${keyword} must contain unique strings`, pointer, keyword);
      seen.add(item);
    }
  ...
  private resolveReferences(): void {
    for (const node of this.nodes.values()) {
      ...
      if (name === undefined) {
        this.fail("SCHEMA_REFERENCE_INVALID", `$ref is not an allowed local definition pointer: ${reference}`, childPointer(node.pointer, "$ref"), "$ref");
      }
      const target = this.definitionPointers.get(name);
      if (target === undefined) {
        this.fail("SCHEMA_REFERENCE_INVALID", `$ref target does not exist: ${reference}`, childPointer(node.pointer, "$ref"), "$ref");
      }
...
function evaluate(
  ...
  if (schema.type !== undefined) {
    const types = typeof schema.type === "string" ? [schema.type] : schema.type as readonly string[];
  ...
  for (const keyword of ["allOf", "anyOf", "oneOf"] as const) {
    ...
    charge(state, branches.length, graph.path, childPointer(schemaPointer, keyword), instancePointer);
    let validBranches = 0;
    let firstBranchFailure: EvaluationFailure | undefined;
    for (let index = 0; index < branches.length; index += 1) {
      const result = evaluateChild(`${childPointer(schemaPointer, keyword)}/${index}`, instance, instancePointer);
  ...
  if (schema.not !== undefined) {
    ...
    if (evaluateChild(childPointer(schemaPointer, "not"), instance, instancePointer).valid) reject("not");
  }
  if (schema.if !== undefined) {
    ...
    if (schema[selected] !== undefined) {
      ...
      retain(evaluateChild(childPointer(schemaPointer, selected), instance, instancePointer).failure);
    }
  }
  ...
  if (isObject(instance)) {
    ...
    if (properties !== undefined) {
      ...
      charge(state, names.length, graph.path, childPointer(schemaPointer, "properties"), instancePointer);
      for (const name of names) {
        if (!Object.hasOwn(instance, name)) continue;
        retain(evaluateChild(
    ...
    if (schema.required !== undefined) {
      ...
      charge(state, names.length, graph.path, childPointer(schemaPointer, "required"), instancePointer);
      for (const name of names) if (!Object.hasOwn(instance, name)) reject("required");
    }
    if (schema.additionalProperties !== undefined) {
      charge(state, instanceKeys.length, graph.path, childPointer(schemaPointer, "additionalProperties"), instancePointer);
    ...
    if (isObject(schema.dependentRequired)) {
      for (const trigger of orderedKeys(schema.dependentRequired)) {
        charge(state, 1, graph.path, childPointer(childPointer(schemaPointer, "dependentRequired"), trigger), instancePointer);
        if (!Object.hasOwn(instance, trigger)) continue;
        const names = schema.dependentRequired[trigger] as readonly string[];
        charge(state, names.length, graph.path, childPointer(childPointer(schemaPointer, "dependentRequired"), trigger), instancePointer);
    ...
    if (isObject(schema.dependentSchemas)) {
      for (const trigger of orderedKeys(schema.dependentSchemas)) {
        charge(state, 1, graph.path, childPointer(childPointer(schemaPointer, "dependentSchemas"), trigger), instancePointer);
        if (!Object.hasOwn(instance, trigger)) continue;
        retain(evaluateChild(
  ...
  if (Array.isArray(instance)) {
    ...
    if (prefixItems !== undefined) {
      charge(state, prefixItems.length, graph.path, childPointer(schemaPointer, "prefixItems"), instancePointer);
    ...
    if (schema.items !== undefined) {
      ...
      for (let index = prefixItems?.length ?? 0; index < instance.length; index += 1) {
        retain(evaluateChild(childPointer(schemaPointer, "items"), instance[index]!, `${instancePointer}/${index}`).failure);
      }
    }
    if (schema.contains !== undefined) {
      let matches = 0;
      for (let index = 0; index < instance.length; index += 1) {
        charge(state, 1, graph.path, childPointer(schemaPointer, "contains"), `${instancePointer}/${index}`);
        if (evaluateChild(childPointer(schemaPointer, "contains"), instance[index]!, `${instancePointer}/${index}`).valid) {
          matches += 1;
        }
      }
  ...
  const result: EvaluationResult = failure === undefined ? { valid: true } : { valid: false, failure };
...
function scalarEqual(value: JsonValue, expected: JsonScalar): boolean {
  return isScalar(value) && (typeof value !== "number" || typeof expected !== "number"
    ? value === expected
    : value === expected || (Object.is(value, -0) && Object.is(expected, 0)) || (Object.is(value, 0) && Object.is(expected, -0)));
}
