...
export function parseCapabilityContract(
  ...
  if (root.$defs !== undefined) {
    ...
    for (const [name, schema] of definitionEntries) {
      if (!DEFINITION_NAME.test(name)) {
        invalid("CAPABILITY_DEFINITION_NAME", `invalid definition name ${JSON.stringify(name)}`, path);
      }
  ...
  for (const [name, methodValue] of methodEntries) {
    ...
    if (!Object.hasOwn(method, "input") || !Object.hasOwn(method, "output") || !Object.hasOwn(method, "errors")) {
      invalid(
        "CAPABILITY_METHOD",
