...
import { captureAuthorClosure, captureAuthorModule } from '../src/project/author-module.js'
...
describe('private project author static closure capture', () => {
  test('captures shared modules once under one deterministic closure identity', async () => {
    await withTemporaryDirectory(async (root) => {
      ...
      try {
        ...
        expect(first.modules.filter(({ projectPath }) => projectPath === 'shared.ts')).toHaveLength(
          1,
        )
  ...
  test('rejects cycles and case-fold collisions', async () => {
    await withTemporaryDirectory(async (root) => {
      ...
      await writeFile(
        join(root, 'jig.ts'),
        ['import "./Name.ts";', 'import "./name.ts";', 'export default {};'].join('\n'),
      )
