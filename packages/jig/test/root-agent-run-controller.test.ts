...
describe('private root Agent Run controller', () => {
  test('renders only the selected transient skill manifest deterministically', () => {
    ...
    expect(first).not.toContain('ANTHROPIC_API_KEY')
    expect(first).not.toContain('PI_API_KEY')
  })
  ...
  test('rejects non-text skill bytes before provider allocation', () => {
    ...
    const manifest: AgentRunSkillManifest = Object.freeze({
      skills: Object.freeze([Object.freeze({ name: 'review', files: Object.freeze([file]) })]),
      fileCount: 1,
      contentBytes: 1,
    })
...
function skillManifest(
  ...
  const skills = values.map(([name, sources]) =>
    Object.freeze({
      name,
      files: Object.freeze(
