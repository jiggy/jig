...
describe('Capability Contract/1', () => {
  test('parses and compiles the normative descriptor with its exact digest', async () => {
    ...
    expect(() => parsed.schemas.get('/methods/read/input')!.validate({}, 'INVALID_INPUT')).toThrow(
      SchemaDiagnostic,
    )
  })
  ...
  test('rejects descriptors above the dedicated byte ceiling before parsing', () => {
    expectCapabilityError(new Uint8Array(CAPABILITY_CONTRACT_LIMITS.bytes + 1), 'CAPABILITY_LIMIT')
  })
  ...
  test('accepts boolean schemas and shares descriptor-root definitions', () => {
    ...
    expect(() => parsed.schemas.get('/methods/call/input')!.validate('', 'INVALID_INPUT')).toThrow(
      SchemaDiagnostic,
    )
    expect(() =>
      parsed.schemas.get('/methods/call/output')!.validate(null, 'INVALID_RESULT'),
  ...
  test('does not turn JSON Schema enum recommendations into extra contract rules', () => {
    ...
    expect(() =>
      parsed.schemas.get('/methods/call/input')!.validate(null, 'INVALID_INPUT'),
    ).toThrow(SchemaDiagnostic)
  ...
  test('applies Schema/1 graph limits across all method roots together', () => {
    ...
    expect(() =>
      parseCapabilityContract(bytes(descriptor({ methods })), 'large.capability.json'),
    ).toThrow(SchemaDiagnostic)
  })
