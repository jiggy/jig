...
describe('private Candidate/5', () => {
  test('separates observed semantics from canonical final activation meaning', () => {
    ...
    expect(artifact.candidate.activationMeaningDigest).toBe(
      privateDomainDigest('JIG-Private-Activation-Meaning/1', {
  ...
  test('pins one valid execution Package/1 in every ready target', () => {
    ...
    expect(second.candidate.observedSemanticDigest).toBe(first.candidate.observedSemanticDigest)
...
describe('private Plan/2', () => {
  ...
  test('rejects mismatched embedded evidence and alternate encodings', () => {
    ...
    expect(() => decodePrivateActivationPlanV2(bytes.subarray(0, bytes.length - 1))).toThrow(
      'not in canonical',
    )
    expect(() =>
      decodePrivateActivationPlanV2(encoder.encode(JSON.stringify(value, null, 2) + '\n')),
    ).toThrow('not in canonical')
    expect(() => decodePrivateActivationPlanV2(Buffer.from(bytes))).toThrow('ordinary Uint8Array')
...
function candidateFixture(paths: readonly string[] = ['flows/run'], extraInertPackages = 0) {
  ...
  const packages: Record<string, JsonValue> = Object.fromEntries(
  ...
  const activationMeaningDigest = privateDomainDigest('JIG-Private-Activation-Meaning/1', {
...
function slottedCandidateFixture(
  slots: Readonly<Record<string, { readonly kind: 'flow'; readonly path: string }>>,
) {
  const packages = {
    'flows/bug': {
...
function expectInvalidPlan(plan: unknown, message: string): void {
  expect(() => decodePrivateActivationPlanV2(withLf(canonicalJson(plan as JsonValue)))).toThrow(
    message,
  )
}
