...
describe("private Package/1 artifact store", () => {
  test("publishes canonical bytes and reacquires them after source disposal", async () => {
    await withStoreAndSource(async (store, source) => {
      ...
      expect(`sha256:${createHash("sha256").update(await readFile(blob)).digest("hex")}`)
      ...
      try {
        expect(new TextDecoder().decode(await reopened.read("flow.ts")))
          .toBe("export default 'old';\n");
  ...
  test("concurrent identical publications converge without staging residue", async () => {
    await withStoreAndSource(async (store, source) => {
      ...
      try {
        ...
        expect(new Set(references.map((reference) => reference.digest))).toEqual(new Set([captured.digest]));
