...
describe('system-owned Linux host paths', () => {
  for (const tool of ['bwrap', 'systemd-run', 'systemctl'] as const) {
    test(`resolves ${tool} through the NixOS system profile without PATH`, async () => {
      ...
      const resolved = await resolvePrivateLinuxHostPath(
        privateLinuxHostToolCandidates(tool),
        async (path) => {
          visited.push(path)
          if (path === `/run/current-system/sw/bin/${tool}`)
            return `/nix/store/fixed-tool/bin/${tool}`
          throw missing()
        },
      )
  ...
  test('does not hide access errors behind another path', async () => {
    const visited: string[] = []
    await expect(
      resolvePrivateLinuxHostPath(privateLinuxHostToolCandidates('bwrap'), async (path) => {
        visited.push(path)
  ...
  test('uses the FHS glibc loader on hosts without nix-ld', async () => {
    expect(
      await resolvePrivateLinuxHostLoader(async (path) => {
