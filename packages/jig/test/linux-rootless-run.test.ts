...
hostileDescribe('private rootless Linux hostile envelope', () => {
  afterAll(async () => {
    ...
    const entries = await Array.fromAsync(
      new Bun.Glob('jig-run-*').scan({ cwd: delegatedCgroup, onlyFiles: false }),
    )
    expect(entries).toEqual([])
    expect(
      (await readdir(tmpdir()))
        .filter(rootlessTemporaryEntry)
        .filter((entry) => !initialRootlessTemporaryState.has(entry)),
    ).toEqual([])
    expect(
      (await readdir('/dev')).filter(
        (entry) => entry.startsWith('.jig-') && entry.endsWith('-devices'),
      ),
    ).toEqual([])
  })
  ...
  test('cancels during prepared admission without starting package code', async () => {
    ...
    try {
      ...
      await expect(owner.admit(controller.signal, async () => controller.abort())).rejects.toThrow(
...
function plan(
  ...
  return {
    ...
    readOnlyMounts: [...host.mounts, { source: fixture, destination: '/package' }],
    command: [
      '/jig-runtime/bun',
      ...['--no-env-file', '--no-install', '--config=/dev/null'],
      '/package/flow.ts',
...
async function waitForFence(backend: PrivateLinuxCgroupBackend, owner: unknown, timeoutMs: number) {
  const deadline = Date.now() + timeoutMs
  for (;;) {
    try {
      return await backend.recoverFence(owner)
    } catch (error) {
      if (!(error instanceof PrivateLinuxFenceUnconfirmedError) || Date.now() > deadline)
        throw error
      await Bun.sleep(20)
    }
  }
