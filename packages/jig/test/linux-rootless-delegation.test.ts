    ...
    for (const dependencies of [
      ...
      orchestrationDependencies({
        ...
        resolveManager: async () => {
          throw new Error('manager detail')
        },
      }),
      orchestrationDependencies({
        acquire: async () => {
        ...
        reexecute: async () => {
          throw new Error('user manager failed')
        },
      }),
    ]) {
      const error = await acquireOrReexecutePrivateRootlessLinux({ dependencies }).then(
        () => undefined,
        (failure) => failure,
      )
...
describe('private transient scope preparation', () => {
  ...
  test('refuses a different scope, pre-existing child, or incomplete process move', async () => {
    ...
    await expect(
      preparePrivateRootlessLinuxScope(UNIT, existingChild.dependencies),
    ).rejects.toThrow()
...
function scopeFixture(): ScopeFixture {
  ...
  fixture.dependencies = {
    ...
    pid: () => 44,
    readText: async (path) => {
      if (path === `${SCOPE}/cgroup.procs`)
        return (
          fixture.processes.map(String).join('\n') + (fixture.processes.length === 0 ? '' : '\n')
        )
