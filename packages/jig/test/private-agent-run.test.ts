...
describe('private Agent Run package-local skill projection', () => {
  ...
  test('rejects malformed, duplicate, unsorted, and unknown selections before reads', async () => {
    ...
    for (const selected of [['Bad'], ['nested/path'], ['coding', 'coding'], ['review', 'coding']]) {
      await expect(projectAgentRunSkills(fixture.package, selected)).rejects.toBeInstanceOf(
        AgentRunValidationError,
      )
    }
    for (const selected of [['missing'], ['nested']]) {
      await expect(projectAgentRunSkills(fixture.package, selected)).rejects.toMatchObject({
        code: 'AGENT_RUN_SKILL_UNKNOWN',
      })
    }
    expect(fixture.reads).toEqual([])
  })
  ...
  test('bounds the complete eager manifest before reading package content', async () => {
    ...
    await expect(projectAgentRunSkills(fileBound.package, ['coding'])).rejects.toMatchObject({
      code: 'AGENT_RUN_SKILL_PROJECTION_LIMIT',
    })
    expect(fileBound.reads).toEqual([])

    const byteBound = capturedMetadata([{ path: 'skills/coding/SKILL.md', size: 8_388_609 }])
    await expect(projectAgentRunSkills(byteBound.package, ['coding'])).rejects.toMatchObject({
      code: 'AGENT_RUN_SKILL_PROJECTION_LIMIT',
    })
  ...
  test('rejects captured content which disagrees with its immutable metadata', async () => {
    const fixture = capturedMetadata([{ path: 'skills/coding/SKILL.md', size: 4 }], {
      'skills/coding/SKILL.md': Uint8Array.of(1, 2, 3),
    })
    await expect(projectAgentRunSkills(fixture.package, ['coding'])).rejects.toMatchObject({
      code: 'AGENT_RUN_SKILL_CONTENT_INVALID',
    })
  })
})

function capturedPackage(tree: Readonly<Record<string, string | Uint8Array>>): {
  readonly package: CapturedPackage
...
function capturedMetadata(
  ...
  const read = async (
    ...
    reads.push(path)
    const value =
      contents[path] ?? new Uint8Array(files.find((file) => file.path === path)?.size ?? 0)
