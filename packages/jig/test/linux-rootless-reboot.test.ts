...
const OTHER_BOOT_ID = CURRENT_BOOT_ID === "00000000-0000-0000-0000-000000000000"
...
describe("private rootless Linux reboot recovery", () => {
  test("releases an active old-boot owner without probing its extinct cgroup", async () => {
    ...
    try {
      ...
      expect(JSON.parse(await readFile(join(fixture.owner.ownerStateDirectory, "final.json"), "utf8")))
      ...
      await expect(lstat(fixture.owner.ownerStateDirectory)).rejects.toMatchObject({ code: "ENOENT" });
    } finally {
  ...
  test("retains an active same-boot owner when its delegated identity is missing", async () => {
    ...
    try {
      ...
      expect(JSON.parse(await readFile(join(fixture.owner.ownerStateDirectory, "claim.json"), "utf8")))
        .toMatchObject({ state: "active" });
      await expect(lstat(join(fixture.owner.ownerStateDirectory, "final.json")))
...
async function syntheticOwner(
  ...
  await writeFile(join(allocation.directory, "claim.json"), `${JSON.stringify({
    allocationDigest: allocation.digest,
