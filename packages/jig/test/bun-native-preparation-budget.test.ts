...
describe("private aggregate Bun preparation budget", () => {
  test("bounds distinct actual preparations and ignores duplicate reservations", () => {
    ...
    try {
      for (let index = 0; index < PRIVATE_BUN_PROJECT_PREPARATION_LIMITS.distinctPackages; index += 1) {
        budget.reserve(`sha256:${index.toString(16).padStart(64, "0")}`, `flows/${index}`);
      }
      budget.reserve(`sha256:${"0".repeat(64)}`, "flows/reused");
      expect(() => budget.reserve(`sha256:${"f".repeat(64)}`, "flows/overflow"))
        .toThrow("distinct dependency preparations");
    } finally {
      budget.dispose();
    }
  });
  ...
  test("accepts the exact aggregate byte ceiling and rejects one byte more", () => {
    ...
    try {
      ...
      expect(() => budget.retain([{ size: 1 }], "flows/overflow"))
        .toThrow("project dependency preparation exceeds");
    } finally {
      budget.dispose();
    }
  });
