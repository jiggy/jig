...
describe('private declaration-source capture', () => {
    ...
    await withProject(async (root) => {
      ...
      try {
        ...
        expect(source.members).toEqual([
          {
            id: 'a',
            projectPath: 'bindings/a.ts',
            membership: 'discovered',
            configuredRoot: 'bindings',
          },
          {
            id: 'z',
            projectPath: 'bindings/z.ts',
            membership: 'discovered',
            configuredRoot: 'bindings',
          },
        ])
  ...
  test('records a missing discovered root but keeps exact membership strict', async () => {
    await withProject(async (root) => {
      ...
      try {
        ...
        await expect(
          captureDeclarationSource(project, {
            kind: 'members',
  ...
  test('rejects malformed names, symlinks, hardlinks, duplicate IDs, and collisions', async () => {
    await withProject(async (root) => {
      ...
      try {
        await expect(
          captureDeclarationSource(third, {
            kind: 'members',
            paths: ['one/linked.ts'],
          }),
      ...
      try {
        ...
        try {
          for (const module of bootstrap.modules) {
            expect(
              closure.modules.find(({ projectPath }) => projectPath === module.projectPath),
            ).toEqual(module)
          }
  ...
  test('retains the complete declaration byte closure after live owners close', async () => {
    await withProject(async (root) => {
      ...
      await writeFile(
        join(root, 'jig.ts'),
        ['import { value } from "./helper.ts";', 'export default { value };'].join('\n'),
      )
      const project = await openPrivateProjectRoot(root)
      ...
      try {
        ...
        try {
          ...
          expect(new TextDecoder().decode(await acquired.read('helper.ts'))).toBe(
