...
describe('private activation admission', () => {
  ...
  test('closes generation lineage, references, and canonical encoding', () => {
    ...
    expect(() => createPrivateActivationAdmission({ ...first, planDigest: 'plan' })).toThrow(
      'plan digest',
    )
    ...
    expect(() =>
      decodePrivateActivationAdmission(encoder.encode(JSON.stringify(first, null, 2) + '\n')),
    ).toThrow('not in canonical')
    expect(() => decodePrivateActivationAdmission(Buffer.from(bytes))).toThrow(
      'ordinary Uint8Array',
    )
  })
})
