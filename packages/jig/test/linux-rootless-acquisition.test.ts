...
describe("private rootless Linux acquisition", () => {
  ...
  for (const [label, mutate] of [
    ["non-cgroup-v2 filesystem", (fixture: Fixture) => { fixture.filesystemType = 0x0102_1994; }],
    ["ambiguous cgroup membership", (fixture: Fixture) => {
      fixture.text.set("/proc/self/cgroup", "0::/one\n1:name=/two\n");
    }],
    ["cgroup namespace root", (fixture: Fixture) => {
      fixture.text.set("/proc/self/cgroup", "0::/payload\n");
    }],
    ["owner mismatch", (fixture: Fixture) => {
      fixture.information.set(DELEGATED, directory(0));
    }],
    ["populated parent", (fixture: Fixture) => {
      fixture.text.set(`${DELEGATED}/cgroup.procs`, "99\n");
    }],
    ["sibling cgroup", (fixture: Fixture) => {
      fixture.directories.set(DELEGATED, ["payload", "unrelated"]);
    }],
    ["missing controller", (fixture: Fixture) => {
      fixture.text.set(`${DELEGATED}/cgroup.subtree_control`, "cpu memory\n");
    }],
    ["missing control file", (fixture: Fixture) => {
      fixture.denied.add(`${CURRENT}/pids.max`);
    }],
    ["setuid Bubblewrap", (fixture: Fixture) => {
      fixture.information.set(BWRAP, file(1_000, 0o4755));
    }],
    ["old Bubblewrap", (fixture: Fixture) => {
      fixture.version = "0.11.0";
    }],
    ["failed Bubblewrap feature probe", (fixture: Fixture) => {
    ...
    test(`fails closed for ${label}`, async () => {
      const fixture = validFixture();
      mutate(fixture);
      const failure = await acquirePrivateRootlessLinux(fixture.dependencies)
