...
import {
  ...
  patchConflicts,
  repairBatch,
} from './fixtures/repair-batch/flows/project/batch.ts'
import { syntheticRepair } from '../../../examples/tested-patch/test/fixture.ts'
...
test('a missing second check set prevents every worker dispatch', async () => {
  ...
  try {
    await cp(
      join(import.meta.dir, '../../../examples/tested-patch/fixtures/log-report'),
      join(root, 'first'),
      {
        recursive: true,
      },
