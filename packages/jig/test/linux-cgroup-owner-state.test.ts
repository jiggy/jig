...
describe('private Linux durable owner allocation', () => {
  test('plans without creating a leaf and cancellation is durable and idempotent', async () => {
    ...
    try {
      ...
      expect(
        normalizePrivateLinuxOwnerStateAllocationIdentity(JSON.parse(JSON.stringify(allocation))),
      ).toEqual(allocation)
  ...
  test('does not erase unexpected caller-owned state or steal an active claim', async () => {
    ...
    try {
      ...
      await writeFile(
        join(active.directory, 'owner.json'),
        `${JSON.stringify({
      ...
      await writeFile(
        join(active.directory, 'claim.json'),
        `${JSON.stringify({
          allocationDigest: active.digest,
          kind: 'private-linux-owner-claim/1',
          state: 'active',
      ...
      expect(
        JSON.parse(await readFile(join(active.directory, 'claim.json'), 'utf8')),
      ).toMatchObject({
        state: 'active',
      })
    } finally {
  ...
  test('resumes an exact marker-backed release after interruption', async () => {
    ...
    try {
      ...
      await writeFile(
        join(allocation.directory, 'release.json'),
        `${JSON.stringify({
          ...fields,
