...
For borrowed agreements, name a package-relative path in the authored channel
port. The compiler emits that reference without reading or inventorying package
files. The caller must validate and distribute the referenced JSON bytes with
the descriptor; Jig's managed workflow resolves only exact references and
tracks borrowed-input freshness separately from generated-file ownership.
Native contract closure limits still apply.
