...
describe("FLOW JSON/1", () => {
  ...
  test("rejects duplicate members before object construction", () => {
    expect(() => decodeJson(textEncoder.encode('{"x":1,"x":2}'))).toThrow(
      JsonViolation,
    );
  });
  ...
  test("detects a raw UTF-8 BOM before decoder stripping", () => {
    expect(() =>
      decodeJson(
        new Uint8Array([0xef, 0xbb, 0xbf, ...textEncoder.encode("{}")]),
      ),
    ).toThrow(JsonViolation);
  });

  test("rejects unsafe integral numbers and escaped lone surrogates", () => {
    expect(() => decodeJson(textEncoder.encode("9007199254740992"))).toThrow(
      JsonViolation,
    );
    expect(() => decodeJson(textEncoder.encode('"\\ud800"'))).toThrow(
      JsonViolation,
    );
