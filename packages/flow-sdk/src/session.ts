...
export class RunSession {
      ...
      if (this.channel === 'open') {
        const code =
          ...
          error instanceof PeerProtocolViolation
            ? 'PROTOCOL_ERROR'
            : 'CHANNEL_LOST'
        this.failChannel(code, errorMessageText(error))
      }
    }
  }
  ...
  private receiveError(
    ...
    try {
      ...
      const failure = new OperationError(operation.code, error.message, operation.details)
  ...
  private async handleRoot(root: Root): Promise<void> {
    ...
    try {
      ...
      if (root.controller.signal.aborted) {
        failure = 'CANCELLED'
      } else if (error instanceof OperationError && isWireOperationErrorCode(error.code)) {
        try {
          failureMessage = snapshotOperationMessage(error.message)
          failureDetails =
            error.details === undefined ? undefined : decodeJson(encodeJson(error.details))
          failure = error.code
        } catch (metadataError) {
  ...
  private async complete(): Promise<void> {
    ...
    try {
      ...
      this.channel = 'failed'
      this.completion.reject(new OperationError('CHANNEL_LOST', errorMessageText(error)))
      return
    }
    this.completion.resolve()
  }

  private failChannel(code: 'PROTOCOL_ERROR' | 'CHANNEL_LOST', message: string): void {
...
function isWireOperationErrorCode(value: unknown): value is WireOperationErrorCode {
...
function rootFailureMessage(failure: WireOperationErrorCode | undefined): string {
  switch (failure) {
    case 'CANCELLED':
