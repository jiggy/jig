  ...
  try {
    if (Array.isArray(value)) {
      ...
      if (
        ...
        throw new Json1Error('array must contain only ordinary indexed data')
      }
      ...
      for (let index = 0; index < value.length; index += 1) {
        ...
        if (field === undefined || !('value' in field) || !field.enumerable) {
          throw new Json1Error('array items must be ordinary data')
        }
        visitJson(field.value, depth + 1, state, false)
      }
      return
    }
    ...
    if (prototype !== Object.prototype && prototype !== null) {
      throw new Json1Error('object must be an ordinary record')
    }
    const keys = Object.keys(object)
    if (Reflect.ownKeys(object).length !== keys.length) {
      throw new Json1Error('object members must be enumerable string data')
    }
    ...
    for (const key of keys) {
      ...
      if (!('value' in field)) throw new Json1Error('object members must be ordinary data')
      visitJson(field.value, depth + 1, state, false)
    }
  } finally {
