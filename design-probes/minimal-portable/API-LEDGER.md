...
| `catalogue.directory("./flows")` | Discover immediate FLOW package children inertly. | [Catalogue discovery](../../docs/spec/project-policy.md#9-catalogues-and-bulk-materialization) | The helper name is documented but not implemented. | Candidate; no recursive or magical discovery. |
| `bindingSources.directory("./bindings")` | Avoid one import per Binding while keeping the source root explicit. | [Default project convention](../../docs/spec/project-policy.md#1-default-project-convention) | Missing directory behavior is already specified as empty. | Candidate. |
...
| `design-api.d.ts` | Type-check hypothetical authoring files without an implementation package. | No runtime concept; experiment scaffolding only. | It intentionally omits Services, host capabilities, attachments, Hooks, and bulk recipes. | Delete when a real SDK exists. |
