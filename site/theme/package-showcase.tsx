...
export function PackageShowcase({ data }: { data: PackageShowcaseData }) {
  ...
  if (import.meta.env.SSG_MD) {
    const block = (source: Source) => `### ${source.file}\n\n\`\`\`${source.lang}\n${source.code}\n\`\`\`\n`
  ...
  function navigate(event: KeyboardEvent<HTMLButtonElement>, index: number) {
    const next = event.key === 'ArrowRight' ? (index + 1) % data.stages.length
      : event.key === 'ArrowLeft' ? (index + data.stages.length - 1) % data.stages.length
      : event.key === 'Home' ? 0 : event.key === 'End' ? data.stages.length - 1 : undefined
  ...
  return <section id="showcase" className="showcase package-showcase" aria-labelledby="package-title">
    <div className="showcase-instrument">
      ...
      <div id="package-panel" role="tabpanel" aria-labelledby={`package-tab-${selected}`} tabIndex={0}>
        <div className="package-heading"><span className="visual-caption">{data.packageName}/</span><span>{data.packageLabel}</span></div>
        ...
        <div className="package-code"><CodeBlockRuntime key={source.file} lang={source.lang} title={source.file} code={source.code} wrapCode shikiOptions={highlighting[theme === 'dark' ? 'dark' : 'light']} /></div>
        {selected === 1 && <><p className="visual-footnote">{data.runtimeNote}</p><details className="package-internals"><summary>{data.internalsTitle}</summary><p>{data.internals}</p></details></>}
        {selected === 2 && <dl className="package-exchange"><div><dt>{data.inputLabel}</dt><dd><code>{data.input}</code></dd></div><div><dt>{data.resultLabel}</dt><dd><code>{data.result}</code></dd></div></dl>}
      </div>
