...
- `flows/repair/` owns the reusable JSON-input specialist, proposal validation,
...
- Optional specialist `settings.restoreCorrections: true` requests a retained
  ...
  but blocks a required correction; missing/malformed receipts or restoration
  errors remain failures. Never fall back to a fresh conversation or retry a
...
- The provisioned Jig host tests run actual candidate tests and CLI commands
  through admitted root, repair, and Agent Flows. Recorded Agent responses prove the
