...
export async function draft(run: Pick<RunContext, "input" | "callEffect">): Promise<RunResult> {
  const result = await run.callEffect({
    ...
    method: "run",
    input: {
      instructions: "Draft or revise the proposal described in the following JSON data. Follow the grounded-drafting Skill. Return the requested structured proposal.\n\n" + JSON.stringify(run.input),
