...
export async function review(run: Pick<RunContext, "input" | "settings" | "callEffect">): Promise<RunResult> {
  ...
  const result = await run.callEffect({
    ...
    method: "run",
    input: {
      instructions: "Review the supplied proposal against the supplied evidence. Follow the evidence-review Skill. Apply this configured focus: " +
