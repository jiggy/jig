...
export async function workshop(run: Pick<RunContext, 'input' | 'callFlow'>): Promise<RunResult> {
  ...
  for (let attempt = 1; attempt <= 2; attempt += 1) {
    ...
    if (drafted.outcome === 'blocked' || drafted.outcome === 'limit') {
      ...
      return finish(
        drafted.outcome,
        'The drafting specialist stopped before completing this attempt.',
      )
    }
    ...
    if (feedback.length === 0) {
      ...
      if (reviewed.outcome === 'blocked' || reviewed.outcome === 'limit') {
        ...
        return finish(
          reviewed.outcome,
          'The review specialist stopped before completing its review.',
        )
      }
      if (reviewed.outcome !== 'done')
        throw new Error('The reviewer returned an unexpected outcome.')
      review = parseReview(reviewed.output)
      feedback = review.issues
      history.push({ attempt, feedback, verdict: review.verdict })
      if (review.verdict === 'approve')
        return finish('done', 'The proposal passed mechanical checks and review.')
      if (review.verdict === 'blocked')
        return finish('blocked', 'The reviewer identified a blocking evidence gap.')
    } else {
...
export function checkProposal(proposal: Proposal, request: Request): string[] {
  ...
  for (const section of proposal.sections) {
    ...
    for (const citation of section.citationIds) {
      if (!sources.has(citation))
        issues.push(`Unknown citation: ${citation} in requirement: ${id}.`)
    }
  }
  for (const { id } of request.requirements) {
    if (!seen.has(id)) issues.push(`Missing requirement: ${id}.`)
  }
  return issues.length <= 128
    ? issues
    : [
        ...issues.slice(0, 127),
  ...
  return [
    ...
    ...proposal.sections.map((section) =>
      [
        ...
        `${markdownText(section.claim)} ${section.citationIds
          .map(
            (id) =>
...
function parseReview(value: JsonValue): Review {
  ...
  if ((verdict === 'approve') !== (issues.length === 0) || blankIssueCount !== 0) {
    throw new TypeError(
      `Review issues must agree with the verdict (${verdict}; ${issues.length} issues; ${blankIssueCount} blank issues).`,
    )
  }
  return { verdict, issues }
}

function reason(value: JsonValue): string {
  return string(object(value).reason)
}
