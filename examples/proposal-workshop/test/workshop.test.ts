  ...
  sections: [
    {
      ...
      claim:
        'Propose a two-hour Tuesday evening pilot for eight weeks; day preference remains uncertain.',
      citationIds: ['E1', 'E3'],
    },
    {
      requirementId: 'staffing',
      claim:
        'Two staff for two hours uses four additional staff-hours weekly, within the eight-hour ceiling; confirm consent and rosters first.',
      citationIds: ['E2'],
    },
    {
      requirementId: 'measurement',
      claim:
        'Record evening attendance and staff-hours, collect feedback, and present a continuation decision after eight weeks; agree a target before starting.',
      citationIds: ['E3'],
    },
  ],
  limitations: [
    'The records are synthetic. Survey interest is not an attendance forecast; rosters and targets need approval.',
  ],
})
...
describe("proposal workshop's mechanical evidence gate", () => {
  ...
  test('rejects duplicate requirement sections and ambiguous evidence IDs', () => {
    ...
    expect(() =>
      parseRequest({ ...fixture, evidence: [fixture.evidence[0]!, fixture.evidence[0]!] }),
  ...
  test('renders supplied source links only after approving review', async () => {
    ...
    expect(result).toMatchObject({
      outcome: 'done',
      output: { attempts: 1, review: { verdict: 'approve' } },
    })
    expect((result.output as any).markdown).toContain(
      '[E2](<https://example.org/library-pilot/staffing>)',
    )
    expect(sample.calls.map(({ slot, operationId }) => [slot, operationId])).toEqual([
      ['drafter', 'draft-1'],
      ['reviewer', 'review-1'],
    ])
  })
  ...
  test('revises an unapproved review once and retains its feedback', async () => {
    ...
    const sample = scenario([
      ...
      done({ verdict: 'revise', issues: feedback }),
      done(proposal()),
      approved(),
    ])
    expect(await sample.run()).toMatchObject({
      ...
      output: {
        attempts: 2,
        history: [
          { attempt: 1, feedback, verdict: 'revise' },
          { attempt: 2, feedback: [], verdict: 'approve' },
        ],
      },
    })
  ...
  test('does not turn exhausted revisions into an approved proposal', async () => {
    ...
    expect(await sample.run()).toMatchObject({
      outcome: 'limit',
      output: { attempts: 2, markdown: '', review: null },
    })
  ...
  test('stops on blocked drafting and limited reviewing without retry', async () => {
    const blocked = scenario([
      { outcome: 'blocked', output: { reason: 'Evidence is contradictory.' } },
    ])
    expect(await blocked.run()).toMatchObject({
      outcome: 'blocked',
      output: { attempts: 1, proposal: null, markdown: '' },
    })
    expect(blocked.calls).toHaveLength(1)
    const limited = scenario([
      done(proposal()),
      { outcome: 'limit', output: { reason: 'Agent budget exhausted.' } },
    ])
    expect(await limited.run()).toMatchObject({
      outcome: 'limit',
      output: { attempts: 1, markdown: '' },
    })
  ...
  test('preserves a blocking review and rejects inconsistent approval', async () => {
    const blocked = scenario([
      ...
      done({ verdict: 'blocked', issues: ['The evidence cannot support the required decision.'] }),
    ])
    expect(await blocked.run()).toMatchObject({
      outcome: 'blocked',
      output: { review: { verdict: 'blocked' }, markdown: '' },
    })
    const inconsistent = scenario([
      done(proposal()),
      done({ verdict: 'approve', issues: ['An unresolved material problem.'] }),
    ])
