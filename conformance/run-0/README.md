...
- `fixtures/framing.json` records raw JSON/0 and framing boundaries;
...
- `python-peer/` independently implements JSON/0 framing and a host peer, then
...
It proves that a graph can sit behind the ordinary Run/0 boundary and exercises
the component-local unwrapping needed to preserve an existing Run/0
operational classification through Sley's structured failure wrapper. This is
integration evidence, not another independent Run/0 peer, a Sley conformance
