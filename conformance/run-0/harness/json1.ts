...
class Parser {
  ...
  private value(depth: number): unknown {
    if (depth > MAX_DEPTH) {
      throw new Json1Error('JSON/0 depth limit exceeded')
    }
    this.nodes += 1
    if (this.nodes > MAX_NODES) {
      throw new Json1Error('JSON/0 node limit exceeded')
    }
  ...
  private string(memberName: boolean): string {
    ...
    while (this.index < this.source.length) {
      ...
      if (!escaped && code === 0x22) {
        ...
        if (bytes > limit) throw new Json1Error('JSON/0 string limit exceeded')
        return value
      }
