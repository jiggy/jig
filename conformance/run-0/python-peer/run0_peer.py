"""Independent stdlib-only FLOW Run/0 host peer used by conformance tests."""
...
class Json1Error(ValueError):
    """A value or frame is outside FLOW JSON/0."""


class ProtocolError(RuntimeError):
    """The component violated the exercised Run/0 protocol."""
...
def _string_size(value: str, limit: int) -> None:
    ...
    if size > limit:
        raise Json1Error("string exceeds the JSON/0 limit")
...
def decode_json1(payload: bytes) -> Any:
    if not payload:
        raise Json1Error("empty JSON/0 document")
...
def require_wire_id(value: Any) -> str:
    if not isinstance(value, str) or _WIRE_ID.fullmatch(value) is None:
        raise ProtocolError("invalid Run/0 request ID")
    try:
        size = len(value.encode("ascii"))
    except UnicodeEncodeError as error:
        raise ProtocolError("invalid Run/0 request ID") from error
    if not 1 <= size <= 128:
        raise ProtocolError("invalid Run/0 request ID")
    return value
...
def require_local_name(value: Any) -> str:
    if not isinstance(value, str) or len(value) > 64 or _LOCAL_NAME.fullmatch(value) is None:
        raise ProtocolError("invalid Run/0 LocalName")
    return value
...
class HostPeer:
    """Blocking host-side Run/0 subprocess peer with bounded frame reads."""
        ...
        *,
        ...
        try:
            self._process = subprocess.Popen(
                ...
                # Run/0 permits diagnostics. A file cannot fill up like an
    ...
    def validate_request(
        ...
        if len(self._component_ids) >= MAX_REQUEST_IDS:
            raise ProtocolError("component exceeded the Run/0 request-ID lifetime limit")
